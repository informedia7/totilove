...
class ImageApprovalController {
    ...
    async disapproveImage(req, res) {
        try {
            ...
            const image = await imageApprovalService.disapproveImage(imageId, adminId);
            res.json({
                success: true,
                message: 'Image moved back to pending review',
                image
            });
        } catch (error) {
            logger.error('Error in disapproveImage controller:', error);
            res.status(500).json({
                success: false,
                error: error.message || 'Failed to disapprove image'
            });
        }
    }
