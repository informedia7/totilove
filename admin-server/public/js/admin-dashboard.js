...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const openAccountSettingsLink = document.getElementById('open-account-settings-link');
    ...
    function closeAdminMenu() {
        ...
        adminMenuDropdown.classList.remove('open');
        adminMenuDropdown.setAttribute('aria-hidden', 'true');
    }

    function openAccountSettingsCard() {
        if (!accountSettingsCard) {
            return;
        }

        accountSettingsCard.style.display = 'block';
        accountSettingsCard.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }

    if (adminAccountTrigger && adminMenuDropdown) {
        adminAccountTrigger.addEventListener('click', function (event) {
            event.preventDefault();
            const isOpen = adminMenuDropdown.classList.contains('open');
            if (isOpen) {
                closeAdminMenu();
                return;
            }

            adminMenuDropdown.classList.add('open');
            adminMenuDropdown.setAttribute('aria-hidden', 'false');
        });
    ...
    if (openAccountSettingsLink) {
        openAccountSettingsLink.addEventListener('click', function (event) {
            event.preventDefault();
            closeAdminMenu();
            openAccountSettingsCard();
        });
    }

    if (togglePasswordChange && passwordChangeSection) {
    ...
    if (cancelPasswordChange && passwordChangeSection && passwordChangeForm && passwordMessage) {
