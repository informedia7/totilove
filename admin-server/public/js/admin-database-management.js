...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const orphanTabImagesBtn = document.getElementById('orphan-tab-images');
    const orphanTabDbBtn = document.getElementById('orphan-tab-db');
    const orphanPanelImages = document.getElementById('orphan-panel-images');
    const orphanPanelDb = document.getElementById('orphan-panel-db');
    const scanOrphanImagesBtn = document.getElementById('scan-orphan-images-btn');
    const orphanImageScanLoading = document.getElementById('orphan-image-scan-loading');
    const orphanImageScanResults = document.getElementById('orphan-image-scan-results');
    const orphanImageScanSummary = document.getElementById('orphan-image-scan-summary');
    const orphanImageScanTable = document.getElementById('orphan-image-scan-table');

    let orphanImageScanState = [];

    function getDeleteSelectedOrphanImagesBtn() {
        return document.getElementById('delete-selected-orphan-images-btn');
    }

    function getDeleteAllOrphanImagesBtn() {
        return document.getElementById('delete-all-orphan-images-btn');
    }

    function setActiveOrphanTab(tab) {
        const isImages = tab === 'images';

        if (orphanPanelImages) {
            orphanPanelImages.style.display = isImages ? 'block' : 'none';
        }
        if (orphanPanelDb) {
            orphanPanelDb.style.display = isImages ? 'none' : 'block';
        }

        if (orphanTabImagesBtn) {
            orphanTabImagesBtn.style.background = isImages ? '#667eea' : '#fff';
            orphanTabImagesBtn.style.color = isImages ? '#fff' : '#3f4a5a';
            orphanTabImagesBtn.style.borderColor = isImages ? '#667eea' : '#d4d8e2';
        }
        if (orphanTabDbBtn) {
            orphanTabDbBtn.style.background = isImages ? '#fff' : '#e67e22';
            orphanTabDbBtn.style.color = isImages ? '#3f4a5a' : '#fff';
            orphanTabDbBtn.style.borderColor = isImages ? '#d4d8e2' : '#e67e22';
        }
    }

    if (orphanTabImagesBtn) {
        orphanTabImagesBtn.addEventListener('click', function() {
            setActiveOrphanTab('images');
        });
    }

    if (orphanTabDbBtn) {
        orphanTabDbBtn.addEventListener('click', function() {
            setActiveOrphanTab('db');
        });
    }

    setActiveOrphanTab('images');

    function setOrphanActionButtonsState() {
        const deleteSelectedOrphanImagesBtn = getDeleteSelectedOrphanImagesBtn();
        const deleteAllOrphanImagesBtn = getDeleteAllOrphanImagesBtn();
        const hasResults = orphanImageScanState.length > 0;
        const selectedCount = getSelectedOrphanTargetKeys().length;

        if (deleteSelectedOrphanImagesBtn) {
            deleteSelectedOrphanImagesBtn.disabled = !hasResults || selectedCount === 0;
            deleteSelectedOrphanImagesBtn.textContent = selectedCount > 0
                ? `Delete Selected (${selectedCount})`
                : 'Delete Selected';
        }

        if (deleteAllOrphanImagesBtn) {
            deleteAllOrphanImagesBtn.disabled = !hasResults;
        }
    }

    function getSelectedOrphanTargetKeys() {
        if (!orphanImageScanTable) {
            return [];
        }

        return Array.from(orphanImageScanTable.querySelectorAll('.orphan-file-checkbox:checked'))
            .map(input => input.value)
            .filter(Boolean);
    }

    async function scanOrphanImageFiles() {
        if (scanOrphanImagesBtn) {
            scanOrphanImagesBtn.disabled = true;
            scanOrphanImagesBtn.textContent = 'Scanning...';
        }

        if (orphanImageScanLoading) {
            orphanImageScanLoading.style.display = 'block';
        }

        try {
            const response = await fetch('/api/corruption/orphan-files', {
                method: 'GET',
            ...
            const data = await response.json();
            if (!response.ok || !data.success) {
                throw new Error(data.error || 'Failed to scan orphaned files');
            }

            orphanImageScanState = data.summary?.orphanedFiles || [];
            renderOrphanImageScanResults(data.summary || {});
            setOrphanActionButtonsState();

            if (typeof showSuccess === 'function') {
                showSuccess(`Scan completed. Found ${orphanImageScanState.length} orphaned image file(s).`);
            }
        } catch (error) {
            console.error('Orphan image scan error:', error);
            if (typeof showError === 'function') {
                showError(error.message || 'Failed to scan orphaned image files.');
            }
        } finally {
            if (orphanImageScanLoading) {
                orphanImageScanLoading.style.display = 'none';
            }
            if (scanOrphanImagesBtn) {
                scanOrphanImagesBtn.disabled = false;
                scanOrphanImagesBtn.textContent = 'Scan Orphaned Images';
            }
        }
    }

    function renderOrphanImageScanResults(summary) {
        if (!orphanImageScanResults || !orphanImageScanSummary || !orphanImageScanTable) {
            return;
        }

        orphanImageScanResults.style.display = 'block';

        const orphanCount = summary.orphanedFilesCount || 0;
        const checkedFiles = summary.checkedFiles || 0;
        const directories = summary.directoriesScanned || [];

        orphanImageScanSummary.innerHTML = `
            <div style="padding: 14px; border-left: 4px solid #667eea; background: #f8f9ff; border-radius: 8px;">
                <p style="margin: 0 0 8px 0; color: #333;">
                    Scanned <strong>${checkedFiles.toLocaleString()}</strong> image file(s) in
                    <strong>${directories.length}</strong> directory(ies).
                </p>
                <p style="margin: 0; color: #333;">
                    Found <strong>${orphanCount.toLocaleString()}</strong> orphaned image file(s).
                </p>
            </div>
        `;

        const rows = orphanImageScanState.map(file => `
            <tr>
                <td style="padding: 8px; text-align:center;">
                    <input type="checkbox" class="orphan-file-checkbox" value="${escapeHtml(file.targetKey)}">
                </td>
                <td style="padding: 8px;">${escapeHtml(file.fileName)}</td>
                <td style="padding: 8px; color:#666;">${escapeHtml(file.relativePath)}</td>
                <td style="padding: 8px;">${escapeHtml(file.reason)}</td>
                <td style="padding: 8px; text-align:center;">
                    <button class="delete-single-orphan-btn" data-target-key="${escapeHtml(file.targetKey)}"
                        style="padding: 6px 10px; border: none; background: #c0392b; color: white; border-radius: 4px; cursor: pointer;">
                        Delete
                    </button>
                </td>
            </tr>
        `).join('') || '<tr><td colspan="5" style="padding: 12px; text-align:center; color:#666;">No orphaned image files found.</td></tr>';

        const actionsFooter = orphanImageScanState.length > 0
            ? `
                <div style="display:flex; gap:10px; justify-content:flex-end; padding: 10px 0 0 0;">
                    <button id="delete-selected-orphan-images-btn"
                        style="padding: 9px 18px; background: #c0392b; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600;" disabled>
                        Delete Selected
                    </button>
                    <button id="delete-all-orphan-images-btn"
                        style="padding: 9px 18px; background: #8e2a1d; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600;" disabled>
                        Delete All Orphaned
                    </button>
                </div>
            `
        ...
        orphanImageScanTable.innerHTML = `
            <div style="overflow-x:auto;">
                <table style="width:100%; border-collapse: collapse; background: #fff;">
                    <thead>
                        <tr style="background:#f0f2f5;">
                            <th style="padding: 8px; text-align:center; width: 70px;">
                                <input type="checkbox" id="select-all-orphan-files" ${orphanImageScanState.length === 0 ? 'disabled' : ''}>
                            </th>
                            <th style="padding: 8px; text-align:left;">File</th>
                            <th style="padding: 8px; text-align:left;">Path</th>
                            <th style="padding: 8px; text-align:left;">Reason</th>
                            <th style="padding: 8px; text-align:center; width: 120px;">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${rows}
    ...
    async function deleteOrphanImageFiles({ targetKeys = [], deleteAll = false } = {}) {
        const requestBody = deleteAll ? { deleteAll: true } : { targetKeys };

        const response = await fetch('/api/corruption/orphan-files/delete', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const data = await response.json();
        if (!response.ok || !data.success) {
            throw new Error(data.error || 'Failed to delete orphaned image files');
        }
    ...
    if (scanOrphanImagesBtn) {
        scanOrphanImagesBtn.addEventListener('click', scanOrphanImageFiles);
    }

    async function handleDeleteSelectedOrphanImages() {
        const deleteSelectedOrphanImagesBtn = getDeleteSelectedOrphanImagesBtn();
        if (!deleteSelectedOrphanImagesBtn) {
            return;
        }

            const targetKeys = getSelectedOrphanTargetKeys();
            ...
            const confirmed = typeof showConfirm === 'function'
                ? await showConfirm(
                    `Delete ${targetKeys.length} selected orphaned image file(s)?`,
                    'Delete Selected Orphaned Images',
                    'Delete Selected',
                    'Cancel',
                    'danger'
                )
                : window.confirm(`Delete ${targetKeys.length} selected orphaned image file(s)?`);

            if (!confirmed) {
                return;
            }

            deleteSelectedOrphanImagesBtn.disabled = true;
            deleteSelectedOrphanImagesBtn.textContent = 'Deleting...';

            try {
                const summary = await deleteOrphanImageFiles({ targetKeys });
                if (typeof showSuccess === 'function') {
                    showSuccess(`Deleted ${summary.deletedFiles || 0} orphaned image file(s).`);
                }
                await scanOrphanImageFiles();
            } catch (error) {
                console.error('Delete selected orphan files error:', error);
                if (typeof showError === 'function') {
                    showError(error.message || 'Failed to delete selected orphaned files.');
                }
            } finally {
                setOrphanActionButtonsState();
            }
    ...
    async function handleDeleteAllOrphanImages() {
        const deleteAllOrphanImagesBtn = getDeleteAllOrphanImagesBtn();
        if (!deleteAllOrphanImagesBtn) {
            return;
        }

            if (orphanImageScanState.length === 0) {
                return;
            }

            const confirmed = typeof showConfirm === 'function'
                ? await showConfirm(
                    `Delete all ${orphanImageScanState.length} scanned orphaned image file(s)?`,
                    'Delete All Orphaned Images',
                    'Delete All',
                    'Cancel',
                    'danger'
                )
                : window.confirm(`Delete all ${orphanImageScanState.length} scanned orphaned image file(s)?`);

            if (!confirmed) {
                return;
            }

            deleteAllOrphanImagesBtn.disabled = true;
            deleteAllOrphanImagesBtn.textContent = 'Deleting...';

            try {
                const summary = await deleteOrphanImageFiles({ deleteAll: true });
                if (typeof showSuccess === 'function') {
                    showSuccess(`Deleted ${summary.deletedFiles || 0} orphaned image file(s).`);
                }
                await scanOrphanImageFiles();
            } catch (error) {
                console.error('Delete all orphan files error:', error);
                if (typeof showError === 'function') {
                    showError(error.message || 'Failed to delete orphaned files.');
                }
            } finally {
                if (deleteAllOrphanImagesBtn) {
                    deleteAllOrphanImagesBtn.textContent = 'Delete All Orphaned';
                }
                setOrphanActionButtonsState();
            }
    }
    ...
    if (orphanImageScanTable) {
        orphanImageScanTable.addEventListener('change', function(event) {
            const target = event.target;
            if (!(target instanceof HTMLElement)) {
                return;
            }

            if (target.id === 'select-all-orphan-files' && target instanceof HTMLInputElement) {
                const checked = target.checked;
                orphanImageScanTable.querySelectorAll('.orphan-file-checkbox').forEach(checkbox => {
                    checkbox.checked = checked;
                });
            }

            if ((target.classList && target.classList.contains('orphan-file-checkbox')) || target.id === 'select-all-orphan-files') {
                const allCheckbox = orphanImageScanTable.querySelector('#select-all-orphan-files');
                const checkboxes = orphanImageScanTable.querySelectorAll('.orphan-file-checkbox');
                const checkedBoxes = orphanImageScanTable.querySelectorAll('.orphan-file-checkbox:checked');
        ...
        orphanImageScanTable.addEventListener('click', async function(event) {
            const target = event.target;
            if (!(target instanceof HTMLElement)) {
                return;
            }

            const deleteBtn = target.closest('.delete-single-orphan-btn');
            if (!deleteBtn) {
                const deleteSelectedBtn = target.closest('#delete-selected-orphan-images-btn');
                if (deleteSelectedBtn) {
                    await handleDeleteSelectedOrphanImages();
                    return;
                }

                const deleteAllBtn = target.closest('#delete-all-orphan-images-btn');
                if (deleteAllBtn) {
                    await handleDeleteAllOrphanImages();
                    return;
                }
            ...
            const key = deleteBtn.getAttribute('data-target-key');
            if (!key) {
                return;
            }
            ...
            const confirmed = typeof showConfirm === 'function'
                ? await showConfirm(
                    'Delete this orphaned image file?',
                    'Delete Orphaned Image',
                    'Delete',
                    'Cancel',
                    'danger'
                )
                : window.confirm('Delete this orphaned image file?');
            ...
            try {
                const summary = await deleteOrphanImageFiles({ targetKeys: [key] });
                if (typeof showSuccess === 'function') {
                    showSuccess(`Deleted ${summary.deletedFiles || 0} orphaned image file(s).`);
                }
                await scanOrphanImageFiles();
            } catch (error) {
                console.error('Delete orphan file error:', error);
                if (typeof showError === 'function') {
                    showError(error.message || 'Failed to delete orphaned image file.');
                }
    ...
    if (cleanupBtn) {
        cleanupBtn.addEventListener('click', async function() {
            const confirmed = typeof showConfirm === 'function'
                ? await showConfirm(
                    'This will permanently delete orphaned database rows only.',
                    'Clean Orphaned Data',
                    'Clean Rows',
                    'Cancel',
                    'danger'
                )
                : window.confirm('Clean orphaned database rows now?');
    ...
    function renderOrphanCleanupResults(summary) {
        ...
        const orphanedFilesCount = fileCleanup.orphanedFilesCount || 0;
        ...
        cleanupSummary.innerHTML = `
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px;">
                ...
                <div style="padding: 16px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #e67e22;">
                    <div style="font-size: 28px; font-weight: 700; color: #e67e22;">${orphanedFilesCount.toLocaleString()}</div>
                    <div style="color: #666; font-size: 13px;">Orphan upload files detected</div>
                </div>
        ...
        cleanupFiles.innerHTML = `
            ...
            <div style="padding: 14px; border-radius: 8px; background: #f8f9fa; border-left: 4px solid #e67e22;">
                ...
                <p style="margin: 0 0 8px 0; color: #444;">
                    Use <strong>Scan Orphaned Images</strong> above to review reasons, then delete one-by-one, selected, or all.
                </p>
...
// Top-level tab switching (same pattern as payments page)
