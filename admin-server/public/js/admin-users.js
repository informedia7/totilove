...
function renderUsers(users) {
    ...
    tbody.innerHTML = users.map(user => `
        <tr id="user-row-${user.id}" class="user-row" data-user-id="${user.id}" tabindex="-1" style="cursor: pointer;">
...
async function saveUserChanges(userId) {
    ...
    try {
        const updateData = {
            ...
            about_me: (v => (!v || v === 'N/A' || v.toLowerCase() === 'not specified') ? null : v)(document.getElementById('editAboutMe').value.trim()),
            partner_preferences: (v => (!v || v === 'N/A' || v.toLowerCase() === 'not specified') ? null : v)(document.getElementById('editPartnerPreferences').value.trim()),
