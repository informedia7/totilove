...
const testDefinitions = [
    ...
    },
    {
        ...
        run: async () => {
            ...
            const requiredIndicators = ['presence-engine.js'];
            const optionalIndicators = ['presence-refresh.js'];
            
            const missingRequired = requiredIndicators.filter(token => !fileBuffer.includes(token));
            const missingOptional = optionalIndicators.filter(token => !fileBuffer.includes(token));

            if (missingRequired.length) {
                throw new Error(`Missing required presence indicators: ${missingRequired.join(', ')}`);
            }
            ...
            return {
                ...
                missingOptional: missingOptional.length > 0 ? missingOptional : null,
