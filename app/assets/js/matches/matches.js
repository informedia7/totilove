        ...
        <div class="online-user-card" data-user-id="${userId}" data-action="view-profile" style="background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(31,38,135,0.08); transition: transform 0.2s; cursor: pointer; position: relative;">
            ...
            <div class="user-image-container" style="position: relative;">
                <img src="${profileImage ? profileImage : defaultImage}"
                     ...
                     loading="lazy">
                ${onlineDot}
                <button class="uc-quick-action uc-quick-action-view matches-eye-btn" data-action="view-profile" data-user-id="${userId}" aria-label="View profile" style="display:none;"><i class="fas fa-eye"></i></button>
            </div>
    ...
    container.addEventListener('click', function(e) {
        ...
        if (!target) {
            ...
            if (card && !e.target.closest('.user-actions') && !e.target.closest('.match-card-checkbox')) {
                // On mobile portrait, toggle eye button visibility on image tap
                const imageContainer = e.target.closest('.user-image-container');
                if (imageContainer) {
                    if (imageContainer.classList.contains('uc-show-actions')) {
                        imageContainer.classList.remove('uc-show-actions');
                    } else {
                        document.querySelectorAll('.user-image-container.uc-show-actions').forEach(c => c.classList.remove('uc-show-actions'));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (matchScoreSlider) {
        // Handle select change - apply filter with scroll preservation
