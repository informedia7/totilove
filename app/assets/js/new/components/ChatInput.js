...
export class ChatInput extends BaseComponent {
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (error.code === 'RECEIVER_PAUSED') {
                this.showNotification('User account is paused', 'warning');
            } else if (error.code === 'RECEIVER_SUSPENDED') {
                this.showNotification('User currently not available', 'warning');
            } else if (error.code === 'EMAIL_VERIFICATION_REQUIRED' || error.message?.includes('EMAIL_VERIFICATION')) {
                this.showNotification('📧 Please verify your email address to send messages', 'warning');
            } else {
