...
(function() {
    ...
    const ActivityManager = {
        ...
        layoutView: 'grid',
        cardsPerRow: 4,
        defaultCardsPerRowOptions: null,
        cardsPerRowViewportMode: 'desktop',
        lastDesktopCardsPerRowValue: '4',
        lastMobileCardsPerRowValue: '2',
        cardsPerRowMediaQuery: null,
        cardsPerRowViewportListenerBound: false,
        tabVisibilityMap: {
            all: ['viewers', 'favorites', 'messages', 'who-liked-me', 'who-i-like', 'blocked'],
            messages: ['messages'],
            views: ['viewers'],
            favorites: ['favorites'],
            likes: ['who-liked-me'],
        ...
        /** Set after first applyActiveTabState so viewport resize does not re-read ?tab= / hash. */
        _initialTabFromUrlDone: false,
        ...
        async init() {
            ...
            this.updateCardsPerRowOptionsForViewport(true);
        ...
        setSectionExpansion(section, shouldExpand, options = {}) {
            ...
            if (messagesList) {
                if (!shouldExpand) {
                    ...
                    messagesList.classList.remove('grid', 'list-view', 'columns-1', 'columns-2', 'columns-3', 'columns-4', 'columns-5', 'columns-6');
                } else if (this.layoutView === 'list') {
                    messagesList.style.display = 'block';
                    messagesList.classList.remove('grid', 'columns-1', 'columns-2', 'columns-3', 'columns-4', 'columns-5', 'columns-6');
                    messagesList.classList.add('list-view');
                } else {
                    messagesList.style.display = 'grid';
                    messagesList.classList.remove('list-view', 'columns-1', 'columns-2', 'columns-3', 'columns-4', 'columns-5', 'columns-6');
                    messagesList.classList.add('grid', `columns-${this.cardsPerRow}`);
                }
            }
        ...
        renderViewers(viewers) {
            this.renderUserCardSection({
                ...
                allowOverlayActionsWhenUnavailable: true,
                metaBuilder: (viewer) => [
                    this.buildMetaRow('far fa-clock', `Viewed ${this.formatTime(viewer.viewed_at)}`)
                ],
                overlayBuilder: (viewer, profile) => {
                    const viewerId = viewer.user_id || viewer.viewer_id || viewer.id || profile.id || '';
                    if (!viewerId) {
                        return '';
                    }
                    return `
                        <button class="remove-viewer-btn" type="button" data-user-id="${viewerId}" title="Remove viewer">
        ...
        renderFavorites(favorites) {
            this.renderUserCardSection({
                ...
                quickActionOverrides: [
                    {
                        ...
                        handler: (profileId) => this.removeFavorite(profileId)
                    }
                ],
                ...
                overlayBuilder: (favorite, profile) => {
                    ...
                    const isMobilePortrait = typeof this.isMobilePortraitViewport === 'function' && this.isMobilePortraitViewport();
                    return `
                        <button class="remove-favorite-btn" type="button" data-user-id="${userId}" title="Remove from favorites">
                            ${isMobilePortrait
                                ? '<i class="fas fa-heart-broken"></i>'
                                : '<i class="fas fa-heart favorite-icon-default"></i><i class="fas fa-heart-broken favorite-icon-hover"></i>'}
                        </button>
                    `;
                }
            });
        ...
        renderMessages(messages) {
            ...
            this.applyLayoutToVisibleSections();
        },
        ...
        renderWhoLikedMe(users) {
            this.renderUserCardSection({
                ...
                quickActionOverrides: [
                    {
                        ...
                        handler: (profileId) => this.hideIncomingLike(profileId)
                    }
                ],
                metaBuilder: (user) => [
                ...
                overlayBuilder: (user, profile) => {
                    ...
                    const isMobilePortrait = typeof this.isMobilePortraitViewport === 'function' && this.isMobilePortraitViewport();
                    return `
                        <button class="unlike-btn" type="button" data-user-id="${userId}" data-action="hide-incoming-like" title="Remove from list">
                            ${isMobilePortrait
                                ? '<i class="fas fa-thumbs-down"></i>'
                                : '<i class="fas fa-thumbs-up thumb-icon-default"></i><i class="fas fa-thumbs-down thumb-icon-hover"></i>'}
                        </button>
                    `;
                }
            });
                ...
                quickActionOverrides: [
                    {
                        ...
                        iconClass: 'fas fa-thumbs-down',
                        ariaLabel: 'Unlike',
                        handler: (profileId) => this.unlikeUser(profileId)
                    }
                ],
                metaBuilder: (user) => [
                    this.buildMetaRow('far fa-clock', `Liked ${this.formatTime(user.liked_at)}`)
        ...
        renderUserCardSection({
            ...
            allowOverlayActionsWhenUnavailable = false,
            quickActionOverrides = [],
            metaBuilder,
            actionsBuilder,
            overlayBuilder
            ...
            const cards = users.map((user) => {
                ...
                const overlay = Array.isArray(overlayResult)
                    ...
                    : (overlayResult || '');
                const allowOverlayActionsClass = allowOverlayActionsWhenUnavailable && overlay
                    ? ' account-status-allow-overlay-actions'
                    : '';

                // Build account status badge
                const isSuspended = profile.is_suspended;
                const isPaused = profile.is_paused;
                const statusBadge = (isSuspended || isPaused)
                    ? `<div class="account-status-badge account-status-unavailable"><i class="fas fa-user"></i> Unavailable right now</div>`
                    : '';
                const statusAttr = isSuspended ? 'data-account-suspended="true"' : isPaused ? 'data-account-paused="true"' : '';

                const blockingOverlay = (isSuspended || isPaused)
                    ? `<div class="account-status-blocker" data-suspended="${isSuspended}" data-paused="${isPaused}"></div>`
                    : '';

                return `
                    <div class="activity-card-wrapper${isSuspended || isPaused ? ' account-status-inactive' : ''}${(isSuspended || isPaused) ? allowOverlayActionsClass : ''}" data-user-id="${profile.id ?? ''}" ${statusAttr}>
                        ${blockingOverlay}
                        ${statusBadge}
            ...
            // Block all interaction on paused/suspended cards via overlay
            container.querySelectorAll('.account-status-blocker').forEach(blocker => {
                const handler = (e) => {
                    e.stopPropagation();
                    e.preventDefault();
                    const msg = 'This profile is temporarily unavailable.';
                ...
                blocker.addEventListener('click', handler);
                blocker.addEventListener('touchend', handler, { passive: false });
            });

            window.setupResultsUserCardEvents(container);
            this.applyQuickActionOverrides(container, quickActionOverrides);
            this.applyLayoutToVisibleSections();
        },

        applyQuickActionOverrides(container, overrides = []) {
            if (!container || !Array.isArray(overrides) || overrides.length === 0) {
                return;
            }

            overrides.forEach((override) => {
                if (!override || !override.action || typeof override.handler !== 'function') {
                    return;
                }

                const selector = `.uc-quick-action[data-action="${override.action}"]`;
                container.querySelectorAll(selector).forEach((btn) => {
                    if (!btn) {
                        return;
                    }

                    const boundKey = `activityOverrideBound_${override.action}`;
                    ...
                    if (icon && override.iconClass) {
                        icon.className = override.iconClass;
                    }
                    if (override.ariaLabel) {
                        btn.setAttribute('aria-label', override.ariaLabel);
                    }
                    ...
                    const onOverride = (event) => {
                        ...
                        const profileId = parseInt(btn.dataset.profileId || btn.closest('.uc-card')?.dataset.profileId || '', 10);
                        if (!profileId) {
                            return;
                        }
                        override.handler(profileId, btn.dataset.username || '', btn);
                    };

                    // Capture-phase to beat existing user-card quick action handlers.
                    btn.addEventListener('click', onOverride, { capture: true });
                    btn.addEventListener('pointerup', onOverride, { capture: true });
                    btn.addEventListener('touchend', onOverride, { capture: true, passive: false });
                });
            });
        },

        setupLayoutControls() {
            const cardsPerRowSelect = document.getElementById('activityCardsPerRow');
            if (cardsPerRowSelect) {
                const parsedValue = Number(cardsPerRowSelect.value);
                if (Number.isFinite(parsedValue) && parsedValue >= 1 && parsedValue <= 6) {
                    this.cardsPerRow = parsedValue;
                }

                this.cacheDefaultCardsPerRowOptions(cardsPerRowSelect);

                cardsPerRowSelect.addEventListener('change', (event) => {
                    const nextValue = Number(event.target.value);
                    if (!Number.isFinite(nextValue) || nextValue < 1 || nextValue > 6) {
                        return;
                    }

                    this.cardsPerRow = nextValue;
                    if (this.cardsPerRowViewportMode === 'mobile') {
                        this.lastMobileCardsPerRowValue = String(nextValue);
                    } else {
                        this.lastDesktopCardsPerRowValue = String(nextValue);
                    }
                    this.applyLayoutToVisibleSections();
                });
            }
            ...
            if (!this.cardsPerRowViewportListenerBound) {
                if (typeof window.matchMedia === 'function') {
                    this.cardsPerRowMediaQuery = window.matchMedia(MOBILE_PORTRAIT_MEDIA_QUERY);
                    const handleViewportChange = () => {
                        this.updateCardsPerRowOptionsForViewport();
                    };

                    if (typeof this.cardsPerRowMediaQuery.addEventListener === 'function') {
                        this.cardsPerRowMediaQuery.addEventListener('change', handleViewportChange);
                    } else if (typeof this.cardsPerRowMediaQuery.addListener === 'function') {
                        this.cardsPerRowMediaQuery.addListener(handleViewportChange);
                    }
                }

                window.addEventListener('resize', () => this.updateCardsPerRowOptionsForViewport());
                this.cardsPerRowViewportListenerBound = true;
            }
        },

        cacheDefaultCardsPerRowOptions(select) {
            if (this.defaultCardsPerRowOptions || !select) {
                return;
            }

            this.defaultCardsPerRowOptions = Array.from(select.options).map((option) => ({
                value: option.value,
                label: option.textContent,
                selected: option.selected
            }));

            const initialSelection = this.defaultCardsPerRowOptions.find((option) => option.selected) || this.defaultCardsPerRowOptions[0];
            if (initialSelection) {
                this.lastDesktopCardsPerRowValue = initialSelection.value;
            }
        },

        setCardsPerRowOptions(select, options, fallbackValue) {
        ...
        isMobilePortraitViewport() {
            ...
            if (!this.cardsPerRowMediaQuery && typeof window.matchMedia === 'function') {
                this.cardsPerRowMediaQuery = window.matchMedia(MOBILE_PORTRAIT_MEDIA_QUERY);
            }

            if (this.cardsPerRowMediaQuery) {
                return this.cardsPerRowMediaQuery.matches;
            }

            return window.innerWidth <= 600 && window.innerWidth <= window.innerHeight;
        },

        updateCardsPerRowOptionsForViewport(forceApply = false) {
            const select = document.getElementById('activityCardsPerRow');
            if (!select) {
                return;
            }

            this.cacheDefaultCardsPerRowOptions(select);
            if (!this.defaultCardsPerRowOptions) {
                return;
            }
            ...
            if (!forceApply && desiredMode === this.cardsPerRowViewportMode) {
                return;
            }

            if (this.cardsPerRowViewportMode === 'desktop' && select.value) {
                this.lastDesktopCardsPerRowValue = select.value;
            }

            this.cardsPerRowViewportMode = desiredMode;

            if (desiredMode === 'mobile') {
                const options = MOBILE_CARDS_PER_ROW_OPTIONS.map((option) => ({
                    ...option,
                    selected: option.value === this.lastMobileCardsPerRowValue
                }));
                this.setCardsPerRowOptions(select, options, this.lastMobileCardsPerRowValue);
                if (!['1', '2'].includes(select.value)) {
                    select.value = '2';
                }
                this.lastMobileCardsPerRowValue = select.value;
            } else {
                const options = this.defaultCardsPerRowOptions.map((option) => ({
                    ...option,
                    selected: option.value === this.lastDesktopCardsPerRowValue
                }));
                this.setCardsPerRowOptions(select, options, this.lastDesktopCardsPerRowValue);
                if (!this.defaultCardsPerRowOptions.some((option) => option.value === select.value)) {
                    select.value = this.lastDesktopCardsPerRowValue || '4';
                }
                this.lastDesktopCardsPerRowValue = select.value;
            }

            this.cardsPerRow = Number(select.value) || (desiredMode === 'mobile' ? 2 : 4);
            this.updateLayoutControlUI();
            this.applyLayoutToVisibleSections();
        },
        ...
        updateLayoutControlUI() {
            document.querySelectorAll('.activity-view-btn').forEach((button) => {
                const isActive = button.dataset.view === this.layoutView;
                button.classList.toggle('active', isActive);
            });

            const cardsPerRowSelect = document.getElementById('activityCardsPerRow');
            if (cardsPerRowSelect) {
                cardsPerRowSelect.disabled = this.layoutView === 'list';
            }
        },

        applyUserCardColumnAttributes(value) {
            const colValue = String(value);
            document.querySelectorAll('.activity-card-grid .uc-card').forEach(card => {
                card.dataset.columns = colValue;
            });
            document.querySelectorAll('.activity-card-grid .uc-image-container').forEach(container => {
                container.dataset.columns = colValue;
            });
        },

        applyLayoutToVisibleSections() {
            const visibleSections = document.querySelectorAll('.activity-section:not(.is-hidden)');
            visibleSections.forEach((section) => {
                const cardGrid = section.querySelector('.activity-card-grid');
                if (cardGrid) {
                    cardGrid.classList.remove('list-view', 'columns-1', 'columns-2', 'columns-3', 'columns-4', 'columns-5', 'columns-6');
                    if (this.layoutView === 'list') {
                        cardGrid.classList.add('list-view');
                        cardGrid.style.removeProperty('display');
                        cardGrid.style.removeProperty('grid-template-columns');
                        cardGrid.style.removeProperty('overflow-x');
                        cardGrid.style.removeProperty('scroll-snap-type');
                    } else {
                        const cols = this.cardsPerRow;
                        cardGrid.classList.add(`columns-${cols}`);
                        cardGrid.style.display = 'grid';
                        cardGrid.style.gridTemplateColumns = `repeat(${cols}, minmax(0, 1fr))`;
                        cardGrid.style.overflowX = 'visible';
                        cardGrid.style.scrollSnapType = 'none';
                        cardGrid.querySelectorAll(':scope > .activity-card-wrapper').forEach(wrapper => {
                            wrapper.style.flex = 'initial';
                            wrapper.style.maxWidth = '100%';
                            wrapper.style.scrollSnapAlign = 'unset';
                            wrapper.style.width = '100%';
                        });
                    }
                }
            });
            this.applyUserCardColumnAttributes(this.layoutView === 'list' ? 1 : this.cardsPerRow);
            visibleSections.forEach((section) => {
        ...
        normalizeProfileForCard(user = {}) {
            ...
            return {
                ...
                is_paused: (user.account_status || 'active') === 'paused',
            };
        },
            ...
            document.addEventListener('click', (e) => {
                ...
                if (e.target.closest('.unlike-btn')) {
                    ...
                    if (userId) {
                        ...
                        if (action === 'hide-incoming-like') {
                            this.hideIncomingLike(userId);
                        } else {
                ...
                if (e.target.closest('.remove-viewer-btn')) {
                    e.preventDefault();
                    e.stopPropagation();
                    const button = e.target.closest('.remove-viewer-btn');
                    const userId = button?.dataset.userId;
                    if (userId) {
        ...
        getInitialTabFromUrl() {
            const validTabs = Object.keys(this.tabVisibilityMap);
            ...
            const fromSection = sectionToTab[rawHash];
            if (fromSection && validTabs.includes(fromSection)) {
                return fromSection;
            }
        ...
        focusDeepLinkedSection(tab) {
            const sections = this.tabVisibilityMap[tab];
            const primarySection = Array.isArray(sections) ? sections[0] : null;
            if (!primarySection) {
                return;
            }

            if (tab === 'messages') {
                this.toggleSection('messages', true);
            }

            const runScroll = () => {
                const el = document.querySelector(`.activity-section[data-section="${primarySection}"]`);
        ...
        applyActiveTabState() {
            if (!this._initialTabFromUrlDone) {
                this._initialTabFromUrlDone = true;
                const urlTab = this.getInitialTabFromUrl();
                if (urlTab) {
                    const tabBtn = document.querySelector(`.activity-tab[data-tab="${urlTab}"]`);
                    if (tabBtn) {
                        this.switchTab(tabBtn);
                        this.focusDeepLinkedSection(urlTab);
                        return;
                    }
                }
            }
        ...
        messageProfileInModal() {
            ...
            // Open universal message modal with the profile real_name and receiverId
        ...
        async removeViewer(userId) {
            try {
                const response = await fetch(`/api/activity/viewers/${userId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-User-ID': this.currentUserId
                    }
                ...
                const data = await response.json();
                if (data.success) {
                    this.showToast(data.message || 'Viewer hidden', 'success');
                    await this.loadViewers();
                } else {
                    this.showToast(data.error || 'Failed to remove viewer', 'error');
                }
            } catch (error) {
                console.error('Error removing viewer:', error);
                this.showToast('Failed to remove viewer', 'error');
            }
        },

        async hideIncomingLike(userId) {
            try {
                const response = await fetch(`/api/activity/who-liked-me/${userId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-User-ID': this.currentUserId
                    }
                ...
                const data = await response.json();
                if (data.success) {
                    this.showToast(data.message || 'Removed from list', 'success');
                    await this.loadWhoLikedMe();
                } else {
                    this.showToast(data.error || 'Failed to remove from list', 'error');
                }
            } catch (error) {
