...
class LanguageMultiSelectManager {
    ...
    createCustomDropdown(select) {
        ...
        const populateList = () => {
            ...
            languagesToShow.forEach(lang => {
                ...
                li.addEventListener('mouseenter', () => li.style.background = 'var(--app-surface-muted, rgba(255,255,255,0.08))');
    ...
    showInlineLanguageList(container, forceOpen = false) {
        ...
        listContainer.style.cssText = 'background: var(--app-surface, #fff); border: 1px solid var(--app-border-color, rgba(0,0,0,0.08)); border-radius: 12px; padding: 1rem; margin-top: 0.5rem; box-shadow: 0 16px 32px rgba(15,23,42,0.22); color: var(--app-text-primary, #111);';
        
        // Header
        listContainer.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding: 1rem; margin: -1rem -1rem 1rem -1rem; background: var(--app-surface-muted, #f5f5f5); border-radius: 12px 12px 0 0; border-bottom: 1px solid var(--app-border-color, rgba(0,0,0,0.08));">
                <h4 style="margin: 0; font-size: 1rem; color: var(--app-text-primary, #111); font-weight: 600;">Select Language</h4>
                <button type="button" class="close-inline-language-list" style="background: none; border: none; font-size: 1.5rem; cursor: pointer; color: var(--app-text-muted, #666); padding: 0; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center;">×</button>
            </div>
        ...
        languagesToShow.forEach(lang => {
            ...
            flagBadge.style.cssText = 'background: var(--app-surface-muted, #f5f5f5); padding: 0.5rem 1rem; border-radius: 8px; font-size: 0.9rem; display: flex; align-items: center; gap: 0.5rem; border: 1px solid var(--app-border-color, rgba(0,0,0,0.08)); box-shadow: 0 1px 3px rgba(15,23,42,0.18); cursor: pointer; transition: all 0.3s ease; color: var(--app-text-primary, #111);';
            flagBadge.dataset.languageId = lang.id;
            
            // Set selected/unselected colors - green for selected
            if (isSelected) {
                flagBadge.style.borderColor = '#22c55e';
                flagBadge.style.background = 'rgba(34, 197, 94, 0.15)';
                flagBadge.style.boxShadow = '0 2px 8px rgba(34, 197, 94, 0.35)';
            }
