...
function initProfileEdit() {
    ...
    if (aboutEditTextarea && aboutEditCounter && aboutEditCount) {
        ...
        autoResizeTextarea(aboutEditTextarea);
    ...
    if (partnerPreferencesTextarea && partnerPreferencesEditCounter && partnerPreferencesEditCount) {
        ...
        autoResizeTextarea(partnerPreferencesTextarea);
        ...
        try {
            ...
            if (result && result.success) {
                ...
                if (data.countries) {
                    ...
                    // Load current selections
                    const normalizePreferredCountries = (items) => {
                        ...
                        return items
                            .map((item) => {
                                if (item && typeof item === 'object') {
                                    const id = item.id != null ? Number(item.id) : (item.country_id != null ? Number(item.country_id) : null);
                                    const name = item.name || item.country_name || '';
                                ...
                                if (!Number.isNaN(id)) {
                                    const country = allCountries.find(c => Number(c.id) === id);
                                    if (country) {
                                        return { id: country.id, name: country.name, emoji: country.emoji || '' };
                                    }
                                }
                                return null;
                            })
                    ...
                    selectedPreferredCountries = normalizePreferredCountries(data.preferred_countries || data.preferredCountries || []);
...
function toggleNumberOfChildrenVisibility(config) {
    ...
    // When user indicates they have children, hide "None" in the count dropdown.
    const noneOption = Array.from(numberSelect.options).find(opt => {
    ...
    if (noneOption) {
        noneOption.hidden = show;
        noneOption.disabled = show;
        if (show && numberSelect.value === noneOption.value) {
            numberSelect.value = '';
        }
    }
