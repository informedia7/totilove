...
// Toggle birthdate info tooltip
function toggleBirthdateInfo(event) {
    const birthdateInfo = document.getElementById('birthdateInfo');
    if (!birthdateInfo) return;

    const isVisible = birthdateInfo.classList.contains('show');
    if (isVisible) {
        birthdateInfo.classList.remove('show');
    } else {
        birthdateInfo.classList.add('show');
        // Close when clicking outside
        setTimeout(() => {
            document.addEventListener('click', function closeTooltip(e) {
                if (!birthdateInfo.contains(e.target) && e.target !== event.target) {
                    birthdateInfo.classList.remove('show');
                    document.removeEventListener('click', closeTooltip);
                }
            });
...
function setupEventListeners() {
    ...
    // Birthdate info button - toggle birthdate info tooltip
    const birthdateInfoBtn = document.getElementById('birthdate-info-btn');
    if (birthdateInfoBtn) {
        birthdateInfoBtn.addEventListener('click', function (event) {
            event.preventDefault();
            event.stopPropagation();
            toggleBirthdateInfo(event);
        });
    }
...
function validateEmail() {
    ...
    if (email.length > 50 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        emailField.classList.add('error');
        emailField.classList.remove('success');
        errorEl.textContent = 'Please enter a valid email (max 50 characters)';
    } else {
        emailField.classList.remove('error');
        errorEl.textContent = '';
    }
...
function getRegisterBirthdateBounds() {
...
function validateBirthdateInputValue(isoValue) {
    ...
    const { minDate, maxDate } = getRegisterBirthdateBounds();
    if (bd > maxDate) {
        return { ok: false, message: 'You must be at least 18 years old' };
    }
    if (bd < minDate) {
        return { ok: false, message: 'Birthdate must be within the last 120 years' };
    }
    return { ok: true };
}
...
function setDateLimits() {
    ...
    const { minDate, maxDate } = getRegisterBirthdateBounds();
    birthdateInput.min = formatLocalDateYYYYMMDD(minDate);
    birthdateInput.max = formatLocalDateYYYYMMDD(maxDate);
}
...
async function onFormSubmit(e) {
    ...
    try {
        if (!validateForm()) {
            ...
            const firstInvalid = document.querySelector('#registerForm select.error, #registerForm input.error');
            if (firstInvalid) {
        ...
        const submitErrEl = document.getElementById('submit-error');
        if (submitErrEl) {
            submitErrEl.textContent = '';
        }
        ...
        if (result.success) {
            ...
            if (result.requiresEmailVerification) {
                    ...
                    <div class="email-verify-shell">
                        <div class="email-verify-card">
                            ...
                            <div class="email-verify-body">
                                <div class="email-verify-icon" aria-hidden="true">
                                    <svg viewBox="0 0 24 24" role="img" focusable="false">
                                        <path d="M3 7.25A2.25 2.25 0 0 1 5.25 5h13.5A2.25 2.25 0 0 1 21 7.25v9.5A2.25 2.25 0 0 1 18.75 19H5.25A2.25 2.25 0 0 1 3 16.75zm2.14-.75L12 11.36l6.86-4.86zM19.5 8l-6.63 4.7a1.5 1.5 0 0 1-1.74 0L4.5 8v8.75a.75.75 0 0 0 .75.75h13.5a.75.75 0 0 0 .75-.75z"></path>
                                    </svg>
                                </div>
                                ...
                                <p class="email-verify-spam">
                                    Didn't receive the email? Check your spam folder or click the link below to resend.
                                </p>
                                <a href="/login" class="email-verify-login">
                                    Go to Login
                                </a>
                                <a href="#" id="resendVerificationLink" class="email-verify-resend">
                ...
                if (result.emailDispatch && result.emailDispatch.success === false) {
                    showNotification('Account created, but verification email was not sent. Use the resend link now.', 'warning');
                } else {
            ...
            const failMsg = result.errors && result.errors.length
                ? result.errors.join(', ')
                : (result.error || 'Unknown error');
            if (submitErrEl && failMsg) {
                submitErrEl.textContent = failMsg;
                submitErrEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
            if (result.errors) {
...
function isRegisterSectionVisible(el) {
    if (!el) {
        return false;
    }
...
function validateForm() {
    ...
    // State/city only required when those dropdowns are shown (countries without states or states without cities omit them).
    ...
    requiredFields.forEach(field => {
        ...
        if (isEmpty) {
            ...
            if (field === 'birthdate') {
                const bdCheck = validateBirthdateInputValue(value);
                if (!bdCheck.ok) {
                    showError(field, bdCheck.message);
                    isValid = false;
                }
            }
        }
    ...
    if (isRegisterSectionVisible(stateContainer) && stateSelect) {
        const sv = stateSelect.value;
        if (!sv || String(sv).trim() === '') {
            showError('state', 'Please select a state or region');
            isValid = false;
        }
    }

    if (isRegisterSectionVisible(cityContainer) && citySelect) {
        const cv = citySelect.value;
