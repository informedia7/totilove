...
function syncResultsUrlFilterParam(key, value) {
    const urlParams = new URLSearchParams(window.location.search);
...
function setupResultsPhotoFilterControl(initialParams) {
    const photoCheckbox = document.getElementById('results-with-photos');
    if (!photoCheckbox) {
        return;
    }

    photoCheckbox.checked = isTrueFlag(initialParams.withImages);

    photoCheckbox.addEventListener('change', function () {
        const nextWithImages = this.checked;
        const nextParams = {
            ...(Object.keys(currentSearchParams).length > 0 ? currentSearchParams : initialParams),
            withImages: nextWithImages
        };
        delete nextParams.page;

        currentPage = 1;
        currentSearchParams = { ...nextParams };

        syncResultsUrlFilterParam('withImages', nextWithImages);
        updateBackToSearchLinks();

        displaySearchParameters(nextParams).catch(() => {
            showNotification('Some search filters could not be displayed.', 'error');
        });

        performSearch(nextParams);
    });
}
...
async function performSearch(params) {
    // Track params for paginated re-fetches; reset to page 1 on a new search
    ...
    try {
        ...
        searchParams.append('page', params.page || currentPage);
        ...
        if (data.success) {
            ...
            const normalizedResults = (data.results || []).map(result => {
                // Spread first so explicit fields (e.g. profile_image) are not overwritten by ...result
                return {
                    ...result,
            ...
            totalSearchResults = data.pagination?.totalResults ?? normalizedResults.length;
            
            document.getElementById('loadingContainer').style.display = 'none';
            document.getElementById('resultsSection').style.display = 'block';
            
            displayResults(normalizedResults, totalSearchResults);
        } else {
...
function displayResults(results, totalCount) {
    ...
    const displayCount = typeof totalCount === 'number' ? totalCount : results.length;
    countEl.textContent = `${displayCount} matches found`;
    ...
    // Validate and filter out invalid profiles (server already pages the results)
    const validProfiles = results.filter(profile => {
        return profile && typeof profile === 'object' && profile.id;
    });
    
    if (validProfiles.length === 0 && results.length > 0) {
        grid.innerHTML = `
            <div class="no-results" style="grid-column: 1 / -1;">
    ...
    const cardsHTML = validProfiles.map(profile => {
        if (window.renderResultsUserCard) {
            const profileForCard = {
                ...profile,
                prefer_small_thumbnail: currentView === 'list'
            };
            return window.renderResultsUserCard(profileForCard, parsedColumnsValue);
        }
...
function showPagination(totalResults) {
    ...
    if (prevButton) prevButton.style.display = currentPage === 1 ? 'none' : '';
    if (nextButton) nextButton.style.display = currentPage === totalPages ? 'none' : '';
}

// Page navigation — calls the server for each page
...
function shouldDefaultMobileCardsToTwo(searchParams = {}) {
    const systemKeys = new Set(['token', 'currentUser', 'userId', 'page']);
    const hasActualFilters = Object.keys(searchParams).some(key => !systemKeys.has(key));

    let cameFromSearchReferrer = false;
    if (document.referrer) {
        try {
            const referrerUrl = new URL(document.referrer);
            cameFromSearchReferrer = referrerUrl.pathname.toLowerCase().includes('/search');
        } catch (_) {
            cameFromSearchReferrer = document.referrer.toLowerCase().includes('/search');
        }
    }

    return cameFromSearchReferrer || hasActualFilters;
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (isResultsMobilePortraitViewport() && shouldDefaultMobileCardsToTwo(searchParams)) {
        resultsLastMobileCardsPerRowValue = '2';
    }

    setupCardsPerRowControl();
    setupResultsPhotoFilterControl(searchParams);
