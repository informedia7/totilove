...
function updateSelectedCountriesDisplay() {
    ...
    if (!container || !countSpan) {
        console.error('❌ Selected countries elements are missing');
        return;
    }
    ...
    if (selectedCountries.length === 0) {
        ...
        allCountriesSpan.className = 'country-badge country-badge-all';
        ...
        selectedCountries.forEach(country => {
            ...
            badge.className = 'country-badge country-badge-selected';
            // Remove country code prefix if present (e.g., 'au Australia' -> 'Australia')
            let displayName = country.name;
            if (/^[a-z]{2} /.test(displayName)) {
                displayName = displayName.substring(3);
            ...
            removeBtn.className = 'country-badge-remove';
            removeBtn.setAttribute('aria-label', `Remove ${displayName}`);
            removeBtn.textContent = '×';
            removeBtn.addEventListener('click', () => removeCountry(country.id));
