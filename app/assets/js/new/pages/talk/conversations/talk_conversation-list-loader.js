    ...
    try {
        ...
        if (data.success) {
            ...
            data.conversations.forEach(conv => {
                ...
                const rawAge = conv.age ?? conv.partner_age ?? conv.other_age;
                let normalizedAge = undefined;
                if (rawAge != null && rawAge !== '') {
                    const n = Number(rawAge);
                    if (Number.isFinite(n) && n > 0 && n < 130) {
                        normalizedAge = Math.round(n);
                    }
                }

                const rawGender = conv.gender ?? conv.other_gender ?? conv.partner_gender;
                conversations[convId] = {
                    id: convId,
                    name: conv.name || 'Unknown User',
                    real_name: (conv.real_name || conv.name || '').trim() || undefined,
                    gender: rawGender != null && rawGender !== '' ? String(rawGender) : null,
                    age: normalizedAge,
                    avatar: conv.avatar || '',
                    ...
                    isDeleted: isDeleted,
                    is_suspended: conv.is_suspended === true || conv.is_suspended === 'true' || conv.is_suspended === 1 || conv.is_suspended === '1',
                    account_status: (conv.account_status || 'active'),
