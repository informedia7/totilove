...
const TALK_AVAILABILITY_CACHE = new Map();

function normalizeTruth(value) {
    return value === true || value === 'true' || value === 1 || value === '1' || value === 't';
}

function resolveConversationAvailability(conversation) {
    const accountStatus = String(conversation?.account_status || '').trim().toLowerCase();
    const isSuspended = normalizeTruth(conversation?.is_suspended);
    const isPaused = accountStatus === 'paused';
    const isUnavailable = Boolean(isSuspended || isPaused);
    return { isUnavailable, isSuspended, isPaused };
}

/** Prefer live TalkState so async availability refresh blocks profile open. */
function openChatHeaderProfileOrNotify(partnerId, event) {
    if (event) {
    ...
    const conversations = TalkState && typeof TalkState.getConversations === 'function'
        ? TalkState.getConversations()
        : (window.conversations || {});

    let conv = null;
    if (TalkState && typeof TalkState.getCurrentConversation === 'function') {
        const key = TalkState.getCurrentConversation();
        const current = key && conversations[key] ? conversations[key] : null;
        if (current && Number(current.partnerId) === numericId) {
            conv = current;
        }
    }
    if (!conv) {
        conv = Object.values(conversations).find((c) => Number(c.partnerId) === numericId) || null;
    }

    const { isUnavailable } = conv ? resolveConversationAvailability(conv) : { isUnavailable: false };
    if (isUnavailable) {
    ...
    if (typeof openProfileModal === 'function') {
        openProfileModal(numericId);
    }
}

function applyChatHeaderAvailabilityUI({ isUnavailable, isSuspended, isPaused }) {
    const chatHeader = document.getElementById('chatHeader');
    const chatName = document.getElementById('chatName');

    if (chatHeader) {
        chatHeader.classList.toggle('chat-header-unavailable', isUnavailable);
        chatHeader.dataset.accountSuspended = isSuspended ? 'true' : 'false';
        chatHeader.dataset.accountPaused = isPaused ? 'true' : 'false';
    }

    if (chatName) {
        const existing = chatName.querySelector('.conversation-unavailable-badge');
        if (existing) existing.remove();

        if (isUnavailable) {
            const badge = document.createElement('span');
            badge.className = 'conversation-unavailable-badge';
            badge.textContent = 'Unavailable';
            const nameInner = chatName.querySelector('.chat-header-name-inner');
            if (nameInner) {
                nameInner.appendChild(badge);
            } else {
                chatName.appendChild(document.createTextNode(' '));
...
async function refreshAvailabilityForPartner(partnerId) {
    const numericId = Number(partnerId);
    if (!Number.isFinite(numericId)) {
        return null;
    }

    const cached = TALK_AVAILABILITY_CACHE.get(numericId);
    if (cached && cached.expiresAt > Date.now()) {
        return cached.value;
    }

    try {
        const response = await fetch(`/api/user/${numericId}?public=1`, { credentials: 'include' });
        if (!response.ok) {
            return null;
        }
        ...
        const value = {
            is_suspended: normalizeTruth(user.is_suspended),
            account_status: String(user.account_status || 'active')
        };
        TALK_AVAILABILITY_CACHE.set(numericId, { value, expiresAt: Date.now() + 60_000 });
        return value;
    } catch (_error) {
        return null;
    }
...
function buildTalkAvatarCandidateUrls(avatarSrc) {
    const candidates = [];
...
function renderChatAvatarImage(container, conversation, indicatorHtml = '') {
    if (!container || !conversation) return;

    container.innerHTML = '';

    const avatarSrc = conversation.avatar || '';
    const availability = resolveConversationAvailability(conversation);
    const letterUnavailable = availability.isUnavailable;

    const appendLetter = () => {
        if (container.querySelector('.conversation-avatar-letter')) {
            return;
        }
        const letter = document.createElement('span');
        letter.className = 'conversation-avatar-letter'
            + (letterUnavailable ? ' conversation-avatar-letter-unavailable' : '');
        letter.setAttribute('aria-hidden', 'true');
        letter.textContent = getTalkAvatarInitialFromRealName(conversation);
        const indicator = container.querySelector('.online-indicator');
        if (indicator) {
            container.insertBefore(letter, indicator);
        } else {
            container.appendChild(letter);
        }
    };

    if (!hasTalkProfilePhoto(avatarSrc)) {
        appendLetter();
        if (indicatorHtml) {
    ...
    const candidates = buildTalkAvatarCandidateUrls(avatarSrc);
    let index = 0;
    const setNextSource = () => {
...
function updateChatHeader(conversation) {
    ...
    if (chatName) {
        ...
        if (isDeleted) {
            ...
            inner.className = 'chat-header-name-inner';
            const nameSpan = document.createElement('span');
            nameSpan.className = availability.isUnavailable ? 'chat-partner-name-unavailable' : 'chat-header-name-text';
            nameSpan.textContent = conversation.name || '';
            inner.appendChild(nameSpan);

            if (typeof window.renderTalkGenderIconHtml === 'function') {
                const iconHtml = window.renderTalkGenderIconHtml(conversation.gender);
                if (iconHtml) {
                    const genderWrap = document.createElement('span');
                    genderWrap.className = 'chat-header-gender-wrap';
                    genderWrap.innerHTML = iconHtml;
                    inner.appendChild(genderWrap);
                }
            }

            const rawAge = conversation.age;
            const ageNum = rawAge != null && rawAge !== '' ? Number(rawAge) : NaN;
            if (Number.isFinite(ageNum) && ageNum > 0 && ageNum < 130) {
                const ageBadge = document.createElement('span');
                ageBadge.className = 'talk-partner-age-badge';
                ageBadge.textContent = String(Math.round(ageNum));
                ageBadge.title = `Age ${ageNum}`;
                inner.appendChild(ageBadge);
            }

            chatName.appendChild(inner);
        }
        applyChatHeaderAvailabilityUI(availability);
        ...
        if (conversation.partnerId && !isDeleted) {
            ...
            chatName.onclick = (e) => {
                openChatHeaderProfileOrNotify(conversation.partnerId, e);
            };
        } else {
    ...
    if (chatAvatar) {
        if (isDeleted) {
            ...
            const indicatorHtml = (!isDeleted && isOnline && presenceEnabled)
                ? `<div class="online-indicator" data-user-id="${conversation.partnerId}" style="display: block;"></div>`
                : '';

            renderChatAvatarImage(chatAvatar, conversation, indicatorHtml);
            chatAvatar.style.display = 'block';
    ...
    // Refresh paused/suspended from API when list payload may be stale
    if (!isDeleted && conversation.partnerId) {
        setTimeout(async () => {
            const latest = await refreshAvailabilityForPartner(conversation.partnerId);
            if (!latest) return;

            const conversations = TalkState ? TalkState.getConversations() : (window.conversations || {});
            const currentKey = TalkState ? TalkState.getCurrentConversation() : (window.currentConversation || null);
            if (!currentKey || !conversations[currentKey] || conversations[currentKey].partnerId != conversation.partnerId) {
                return;
            }

            conversations[currentKey].is_suspended = latest.is_suspended;
            conversations[currentKey].account_status = latest.account_status;
            if (TalkState && typeof TalkState.setConversations === 'function') {
                TalkState.setConversations(conversations);
            } else {
                window.conversations = conversations;
            }

            const refreshedAvail = resolveConversationAvailability(conversations[currentKey]);
            applyChatHeaderAvailabilityUI(refreshedAvail);
        }, 0);
    }
}
