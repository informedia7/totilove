...
async function loadMessages(conversation, options = {}) {
    ...
    const parseReactionArray = (value) => {
    ...
    const normalizeReactionList = (value) => {
        return parseReactionArray(value)
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const availabilityMessage = (typeof window.resolveTalkAvailabilityMessage === 'function')
                ? window.resolveTalkAvailabilityMessage(payload)
                : null;

            if (availabilityMessage && typeof showNotification === 'function') {
                showNotification(availabilityMessage, 'warning');
        ...
        if (data.success) {
            ...
            const messages = data.messages.map((msg) => {
                ...
                const normalizedMyReactions = normalizeReactionList(msg.my_reactions);
                const normalizedReactions = normalizeReactionSummary(
                    typeof msg.reactions !== 'undefined' ? msg.reactions : msg.reactions_summary
                );
                ...
                const mappedMessage = {
                    ...
                    my_reaction: (typeof msg.my_reaction === 'string' && msg.my_reaction.trim()) ? msg.my_reaction.trim() : null,
                    my_reactions: normalizedMyReactions,
