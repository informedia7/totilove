...
function getSavedMessagePreview(partnerId) {
    try {
        const savedMessages = CONFIG.USERS.SAVED_MESSAGES || [];
        const normalizedPartnerId = parseInt(partnerId);
        const conversationSavedMessages = savedMessages.filter(msg => {
            const normalizedConversationId = parseInt(msg.conversationId);
            return !isNaN(normalizedConversationId) && normalizedConversationId === normalizedPartnerId;
        });
