...
class SearchResults {
    ...
    createMessageItem(msg, conv, query) {
        ...
        try {
            ...
            const avatarImg = item.querySelector('.search-result-avatar img');
            if (avatarImg) {
                this.bindAvatarFallback(avatarImg, avatar, senderName);
            }
    ...
    createAvatarHTML(avatarSrc, name) {
        ...
        try {
            const candidates = this.buildAvatarCandidateUrls(avatarSrc);
            if (!candidates.length) {
                return this.createAvatarLetterHTML(name);
            }
            return `<img src="${this.escapeHtml(candidates[0])}" alt="" />`;
        } catch (error) {
    ...
    createAvatarLetterHTML(name) {
        const initial = this.escapeHtml(this.getAvatarInitial(name));
        return `<span class="search-result-avatar-letter" aria-hidden="true">${initial}</span>`;
    }
    ...
    buildAvatarCandidateUrls(avatarSrc) {
        const candidates = [];
        ...
        const normalizedSrc = typeof window.normalizeTalkAvatarUrlToPath === 'function'
            ...
            : (avatarSrc || '').trim();
        const isUploadPath = normalizedSrc &&
            (normalizedSrc.startsWith('/uploads/') || normalizedSrc.startsWith('uploads/'));
    ...
    bindAvatarFallback(imgElement, avatarSrc, displayName) {
        ...
        imgElement.addEventListener('error', () => {
            if (index >= candidates.length) {
                ...
                if (container) {
                    container.innerHTML = this.createAvatarLetterHTML(displayName);
                }
                return;
            }
