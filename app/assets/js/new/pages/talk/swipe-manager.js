(function (window, document) {
    ...
    class SwipeManager {
        ...
        matchesScrollableTarget(target) {
            if (!target || !target.closest || !Array.isArray(this.options.scrollableSelectors)) {
                return false;
            }

            for (let index = 0; index < this.options.scrollableSelectors.length; index += 1) {
                const selector = this.options.scrollableSelectors[index];
                if (!selector) {
                    continue;
                }
        ...
        onTouchStart(event) {
            if (
                ...
                this.matchesIgnoredTarget(event.target) ||
                this.matchesScrollableTarget(event.target)
            ) {
                this.resetGesture();
                return;
            }
