(function (window, document) {
    ...
    /**
     * Gender glyph HTML matching universal user card (fa-venus / fa-mars + uc-gender-*).
     */
    function renderTalkGenderIconHtml(gender) {
        if (gender === undefined || gender === null || gender === '') {
            return '';
        }
        const genderLower = String(gender).toLowerCase();
        const icon = genderLower.includes('female') || genderLower === 'f' ? 'fa-venus' : 'fa-mars';
        const className = genderLower.includes('female') || genderLower === 'f' ? 'uc-gender-female' : 'uc-gender-male';
        return `<i class="fas ${icon} uc-gender-icon ${className}" aria-hidden="true"></i>`;
    }

    /** Same-origin absolute /uploads URLs → path so avatar candidate builders can pick _thumb_small first. */
    function normalizeTalkAvatarUrlToPath(avatarSrc) {
    ...
    window.normalizeTalkAvatarUrlToPath = normalizeTalkAvatarUrlToPath;
    window.renderTalkGenderIconHtml = renderTalkGenderIconHtml;
})(window, document);
