(function (window, document) {
    ...
    function showIncomingMessageToast(senderName, senderId) {
        ...
        const safeName = String(senderName || 'Someone');
        const senderIdStr = senderId != null ? String(senderId).trim() : '';
        const safeSenderId = senderIdStr && /^\d+$/.test(senderIdStr) ? parseInt(senderIdStr, 10) : null;
        ...
        notification.innerHTML = `
            ...
            <div style="flex:1;min-width:0;overflow:hidden;">
                ...
                <div style="font-size:0.82rem;opacity:0.75;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">From ${safeName}</div>
            </div>
        ...
        notification.addEventListener('click', () => {
            ...
            const url = safeSenderId
                ? `/talk?user=${encodeURIComponent(safeSenderId)}${token ? `&token=${encodeURIComponent(token)}` : ''}`
