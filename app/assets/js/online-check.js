...
(function() {
    ...
    /**
 * Process queued requests in batches to avoid rate limiting
 */
    async function processRequestQueue() {
    if (requestQueue.length === 0) return;
    
    const batch = requestQueue.splice(0, 10); // Process up to 10 requests at a time
    if (batch.length === 0) return;
    
    try {
        const userIds = batch.map(req => req.userId);
        ...
        if (data.success && data.statuses) {
            // Resolve all pending requests with their status data
            batch.forEach(req => {
                const status = data.statuses[req.userId] || {
                    userId: req.userId,
                    isOnline: false,
                    lastSeen: null
                };
    ...
    if (requestQueue.length > 0) {
        setTimeout(processRequestQueue, BATCH_DELAY);
    }
}
 ...
 * @returns {Promise<Object>} - Promise that resolves with user status data
 */
    function queueUserStatusRequest(userId) {
    return new Promise((resolve, reject) => {
        ...
        // Start processing if this is the first request in the queue
 ...
 */
    async function retryRequest(requestFn, attempt = 0) {
    try {
        return await requestFn();
    } catch (error) {
        if (attempt < MAX_RETRIES && (error.status === 429 || error.message.includes('429'))) {
 ...
 */
    ...
    try {
        ...
        console.warn(`Failed to check online status for user ${userId}:`, error);
        return false;
    }
}
 ...
 */
    ...
    try {
        ...
        return statusData.lastSeen ? new Date(statusData.lastSeen) : null;
    } catch (error) {
        console.warn(`Failed to get last seen for user ${userId}:`, error);
        return null;
    }
}
