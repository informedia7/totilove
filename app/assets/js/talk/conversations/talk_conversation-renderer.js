...
function renderConversations() {
    ...
    // Register online indicators with instantStatusManager after rendering
    setTimeout(() => {
...
function createConversationElement(conversation) {
    ...
    div.onclick = (event) => {
        if (typeof selectConversation === 'function') {
            selectConversation(conversation.id, event);
        }
    };
    ...
    div.innerHTML = `
        <div class="conversation-avatar" style="${blockedStyle}">
            ...
            ${!isDeleted ? `<div class="online-indicator" data-user-id="${conversation.partnerId}" style="display: ${isOnline ? 'block' : 'none'};"></div>` : ''}
...
/**
 * Register online indicators with instantStatusManager
...
function registerOnlineIndicators() {
    if (window.instantStatusManager && window.instantStatusManager.isInitialized) {
        ...
        // Retry after a short delay if instantStatusManager isn't ready
        setTimeout(() => {
            if (window.instantStatusManager && window.instantStatusManager.isInitialized) {
                registerOnlineIndicators();
            }
