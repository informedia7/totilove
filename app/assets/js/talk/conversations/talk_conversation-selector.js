            ...
            if (isValidImagePath) {
                ...
                if (!conversation.avatar || typeof conversation.avatar !== 'string' || conversation.avatar.length <= 1) {
                    ...
                    chatAvatar.innerHTML = `
                        ...
                        ${!isDeleted && isOnline ? `<div class="online-indicator" data-user-id="${conversation.partnerId}" style="display: block;"></div>` : ''}
                    `;
                    ...
                    chatAvatar.innerHTML = !isDeleted && isOnline ? `<div class="online-indicator" data-user-id="${conversation.partnerId}" style="display: block;"></div>` : '';
...
async function selectConversation(conversationId, event) {
    ...
    // On mobile: hide sidebar and make chat area full screen
    if (window.innerWidth <= 768) {
        ...
        if (chatArea) {
            ...
            chatArea.style.height = 'calc(100vh - 50px)';
            chatArea.style.zIndex = '100';
            chatArea.style.background = 'white';
        }
        // Show back button in header when conversation is selected
        if (backBtn) {
    ...
    if (chatAvatar) {
        ...
        if (!isDeleted && typeof registerOnlineIndicators === 'function') {
            setTimeout(() => {
...
function goBackToConversations() {
    ...
    // Show sidebar and reset chat area on mobile
    const sidebar = document.querySelector('.sidebar');
    const chatArea = document.querySelector('.chat-area');
    const backBtn = document.getElementById('chatBackBtn');
    ...
    if (chatArea) {
        // Reset all inline styles that were set by selectConversation
    ...
    // Hide back button when returning to conversation list
    if (backBtn) {
