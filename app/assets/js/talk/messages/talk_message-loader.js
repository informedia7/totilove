...
async function loadMessages(conversation, options = {}) {
    let { forceRefresh = false, offset = 0, limit = null, prepend = false, before = null, beforeId = null } = options;
    ...
    try {
        if (typeof checkIfBlocked === 'function') {
            ...
            if (isBlocked) {
                ...
                // On mobile: show sidebar and reset chat area when no conversation selected
    ...
    try {
        ...
        if (data.success) {
            ...
            // Handle message merging for pagination
            // Get current conversation ID from TalkState to find the right conversation
            const currentConversationId = TalkState.getCurrentConversation();
            
            // Find conversation by partnerId (since conversations are stored by partnerId as key)
