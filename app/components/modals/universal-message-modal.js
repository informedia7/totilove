...
async function sendMessage() {
    ...
    try {
        ...
        sendBtn.innerHTML = '<i class="fas fa-spinner fa-spin" aria-hidden="true"></i><span class="sr-only">Sending...</span>';
        ...
        if (response.ok) {
            ...
            if (data.success) {
                ...
                if (data.code === 'RECEIVER_SUSPENDED') {
                    showToast('User currently not available', 'warning');
                    return;
                }
                if (data.code === 'RECEIVER_PAUSED') {
                    showToast('User account is paused', 'warning');
                    return;
                }
            ...
            if (errorData.code === 'RECEIVER_SUSPENDED') {
                showToast('User currently not available', 'warning');
                return;
            }
            if (errorData.code === 'RECEIVER_PAUSED') {
                showToast('User account is paused', 'warning');
                return;
            }
        ...
        sendBtn.innerHTML = `<i class="fas fa-paper-plane" aria-hidden="true"></i><span id="send-btn-text" class="sr-only">${isReply ? 'Send Reply' : 'Send Message'}</span>`;
    }
}
