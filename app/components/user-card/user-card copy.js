...
const DEFAULT_IMAGES = {
  ...
  class UserCard {
    ...
    handleCardClick(event) {
      ...
      if (this.handleMatchesMobilePortraitEye(event, card, profileId)) {
        return;
      }
    ...
    isMatchesPageContext() {
      const path = (window.location && window.location.pathname) ? window.location.pathname.toLowerCase() : '';
      return path.endsWith('/matches') || path.endsWith('/matches.html') || path.includes('/matches/');
    }

    openFullProfileFromEye(profileId, card) {
      this.viewProfile(profileId);
      if (card) {
        this.showActionFeedback(card, 'view');
      }
    }

    handleMatchesMobilePortraitEye(event, card, profileId) {
      if (!this.isMatchesPageContext() || !this.isMobilePortraitViewport()) {
        return false;
      }

      const eyeAction = event.target.closest('.uc-quick-action-view');
      if (eyeAction) {
        document.querySelectorAll('.uc-image-container.uc-show-actions').forEach(container => {
          container.classList.remove('uc-show-actions');
        });
        event.stopPropagation();
        this.openFullProfileFromEye(profileId, card);
        return true;
      }
