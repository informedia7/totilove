...
function toggleGenderInfo(event) {
    ...
    // Use CSS class if available, otherwise fall back to inline style
    const hasShowClass = info.classList.contains('show');
    
    if (!hasShowClass && (info.style.display === 'none' || info.style.display === '')) {
        info.classList.add('show');
        // Also set inline style for backward compatibility
        if (info.classList.contains('info-tooltip')) {
            // CSS handles display via .show class
        } else {
        ...
        const closeTooltip = (e) => {
            if (!info.contains(e.target) && e.target.id !== 'gender-info-icon' && !e.target.closest('button[onclick*="toggleGenderInfo"]') && !e.target.closest('#gender-info-btn')) {
...
function toggleNameInfo(event) {
    ...
    if (!hasShowClass && (info.style.display === 'none' || info.style.display === '')) {
        ...
        // Close tooltip when clicking outside
        const closeTooltip = (e) => {
            if (!info.contains(e.target) && e.target.id !== 'name-info-icon' && !e.target.closest('button[onclick*="toggleNameInfo"]') && !e.target.closest('#name-info-btn')) {
                info.classList.remove('show');
                info.style.display = 'none';
                document.removeEventListener('click', closeTooltip);
            }
