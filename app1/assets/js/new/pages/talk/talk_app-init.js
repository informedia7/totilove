...
async function initApp() {
    ...
    // Wait for search system to initialize if needed
    if (typeof populateSenderFilterFromConversations === 'function') {
        if (window.searchSystemInitialized && window.searchController?.modules?.filters) {
            // Search system already initialized, call directly
            populateSenderFilterFromConversations();
        } else {
            // Wait for search system initialization
            const waitForSearch = () => {
                if (window.searchSystemInitialized && window.searchController?.modules?.filters) {
                    populateSenderFilterFromConversations();
                } else {
            ...
            window.addEventListener('searchSystemInitialized', () => {
                populateSenderFilterFromConversations();
            }, { once: true });
    ...
    if (talkSocketsEnabled() && window.io && typeof window.io === 'function') {
        ...
        console.info('[Talk] WebSocket listeners skipped (socket transport disabled)');
    }
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.filter-btn').forEach(tab => {
        tab.addEventListener('click', async (e) => {
            ...
            if (targetFilter === 'saved' && typeof refreshSavedMessages === 'function') {
                await refreshSavedMessages();
            }

            if (typeof filterConversations === 'function') {
                filterConversations(targetFilter);
            }
        });
    });
