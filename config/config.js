...
module.exports = {
    ...
    // Redis Configuration (REDIS_URL wins when set — used by Socket.IO adapter helpers)
    redis: {
    ...
    // WebSocket Configuration (Socket.IO server uses createEngineIoCorsDelegate — not this block)
    websocket: {
        cors: {
            methods: ['GET', 'POST']
        },
    ...
    session: {
        secret: process.env.SESSION_SECRET || (process.env.NODE_ENV === 'production' ? '' : 'lightning-secret-key'),
    ...
    security: {
        bcryptRounds: 12,
        jwtSecret: process.env.JWT_SECRET || (process.env.NODE_ENV === 'production' ? '' : 'jwt-secret-key'),
        jwtExpiresIn: '1h',
        corsOrigins: (() => {
