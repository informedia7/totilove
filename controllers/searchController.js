...
class SearchController {
    constructor() {
        ...
        this.accountStatusSchemaReady = false;
        this.accountStatusSchemaPromise = null;
    }

    async ensureAccountStatusSchema() {
        if (this.accountStatusSchemaReady) {
            return;
        }

        if (!this.accountStatusSchemaPromise) {
            this.accountStatusSchemaPromise = (async () => {
                await this.db.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS account_status VARCHAR(16) NOT NULL DEFAULT 'active'");
                await this.db.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS is_suspended BOOLEAN DEFAULT false');
                await this.db.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS paused_at TIMESTAMP WITH TIME ZONE');
                await this.db.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS pause_reason TEXT');
                await this.db.query("UPDATE users SET account_status = 'active' WHERE account_status IS NULL OR TRIM(account_status) = ''");
                await this.db.query('UPDATE users SET is_suspended = false WHERE is_suspended IS NULL');
            })();
        }

        await this.accountStatusSchemaPromise;
        this.accountStatusSchemaReady = true;
    }

    async searchUsers(req, res) {
        try {
            await this.ensureAccountStatusSchema();
                    ...
                    ua.about_me,
                                        COALESCE((
                                                ...
                                                WHERE ui2.user_id = u.id
                                                    ...
                                                    AND ui2.approval_status = 'approved'
                                        ), 0) as photo_count,
            ...
            if (relationshipType && 
                ...
                if (/^\d+$/.test(String(relationshipType).trim())) {
                    query += ` AND up.relationship_type_id = $${paramIndex}`;
                    queryParams.push(parseInt(relationshipType, 10));
                    paramIndex++;
                } else {
                    query += ` AND up.relationship_type_id IN (
                        SELECT id
                        FROM user_relationship_type_reference
                        WHERE LOWER(TRIM(COALESCE(display_name, name))) = LOWER(TRIM($${paramIndex}))
                           OR LOWER(TRIM(name)) = LOWER(TRIM($${paramIndex}))
                    )`;
            ...
            if (smoking && smoking !== 'any' && smoking !== '') {
                countQuery += ` AND ua.smoking_id = $${countParamIndex}`;
                countParams.push(smoking);
                countParamIndex++;
            }
            
            if (drinking && drinking !== 'any' && drinking !== '') {
                countQuery += ` AND ua.drinking_id = $${countParamIndex}`;
            ...
            if (relationshipType && 
                ...
                if (/^\d+$/.test(String(relationshipType).trim())) {
                    countQuery += ` AND up.relationship_type_id = $${countParamIndex}`;
                    countParams.push(parseInt(relationshipType, 10));
                    countParamIndex++;
                } else {
                    countQuery += ` AND up.relationship_type_id IN (
                        SELECT id
                        FROM user_relationship_type_reference
                        WHERE LOWER(TRIM(COALESCE(display_name, name))) = LOWER(TRIM($${countParamIndex}))
                           OR LOWER(TRIM(name)) = LOWER(TRIM($${countParamIndex}))
                    )`;
