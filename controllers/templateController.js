...
class TemplateController {
    ...
    async loadUserData(userId, templateName, activeTab) {
        const result = await this.db.query(
            `
                   ...
                   ua.user_id as ua_user_id, ua.about_me, ua.about_partner as partner_preferences, ua.height_cm, ua.weight_kg,
                           ua.smoking_id, ua.drinking_id, ua.exercise_habits_id,
                   ...
                   up.preferred_english_ability,
                   up.relationship_type_id::text as relationship_type,
            ...
            LEFT JOIN user_smoking_preferences sp ON ua.smoking_id = sp.id
            LEFT JOIN user_drinking_preferences dp ON ua.drinking_id = dp.id
        ...
        try {
            const interestsResult = await this.db.query(
                `
                    SELECT 
                        ...
                        uic.color
                    FROM users u
                    JOIN user_interests_multiple uim ON uim.user_id = u.id
                    JOIN user_interest_categories uic ON uic.id = uim.interest_id
                    WHERE u.id = $1
                    ORDER BY uic.name
                `,
                [userId]
            ...
            // Optional interest tables may be missing; ignore failures
        }
        ...
        try {
            ...
            // Optional hobby tables may be missing; ignore failures
        }
        ...
        const formatPreferenceDisplay = async (value, {
            lookupTables = [],
            zeroLabel = 'Not important',
            ...
            if (normalized === '0' || normalized.toLowerCase() === 'not important') {
                return zeroLabel;
            }
            if (/^\d+$/.test(normalized) && lookupTables.length > 0) {
                const lookupName = await resolveLookupName(Number(normalized), lookupTables);
                if (lookupName) {
                    return lookupName;
                }
            }
        ...
        // Height displays
        const isHeightOther = user.height_cm === 0 || user.height_cm === '0';
                    const heightDisplay = isHeightOther ? 'Other' : this.templateUtils.formatHeight(user.height_cm);
        ...
        const heightFullDisplay = isHeightOther
            ? 'Other'
            : (user.height_cm 
        ...
        const relationshipTypeDisplay = await formatPreferenceDisplay(user.relationship_type, {
            lookupTables: ['user_relationship_type_reference'],
            zeroLabel: 'Not important'
        });

        const [
            preferredExerciseDisplay,
            preferredBodyTypeDisplay,
            preferredEducationDisplay,
            preferredReligionDisplay,
            preferredSmokingDisplay,
            preferredDrinkingDisplay,
            preferredChildrenDisplay,
            preferredNumberOfChildrenDisplay,
            preferredEyeColorDisplay,
            preferredHairColorDisplay,
            preferredEthnicityDisplay,
            preferredOccupationDisplay,
            preferredIncomeDisplay,
            preferredMaritalStatusDisplay,
            preferredLifestyleDisplay,
            preferredBodyArtDisplay,
            preferredEnglishAbilityDisplay
        ] = await Promise.all([
            formatPreferenceDisplay(user.preferred_exercise, {
                lookupTables: ['user_exercise_habits', 'exercise_habits']
            }),
            formatPreferenceDisplay(user.preferred_body_type, {
                lookupTables: ['user_body_types', 'body_types', 'body_type_options']
            }),
            formatPreferenceDisplay(user.preferred_education, {
                lookupTables: ['user_education_levels', 'user_education', 'education_levels', 'education']
            }),
            formatPreferenceDisplay(user.preferred_religion, {
                lookupTables: ['user_religions', 'religions', 'user_religion_options']
            }),
            formatPreferenceDisplay(user.preferred_smoking, {
                lookupTables: ['user_smoking_preferences', 'smoking_preferences']
            }),
            formatPreferenceDisplay(user.preferred_drinking, {
                lookupTables: ['user_drinking_preferences', 'drinking_preferences']
            }),
            formatPreferenceDisplay(user.preferred_children, {
                lookupTables: ['user_children_preferences', 'user_have_children_statuses', 'have_children_statuses', 'have_children_status']
            }),
            formatPreferenceDisplay(user.preferred_number_of_children, {
                lookupTables: ['user_number_of_children', 'number_of_children']
            }),
            formatPreferenceDisplay(user.preferred_eye_color, {
                lookupTables: ['user_eye_colors', 'eye_colors']
            }),
            formatPreferenceDisplay(user.preferred_hair_color, {
                lookupTables: ['user_hair_colors', 'hair_colors']
            }),
            formatPreferenceDisplay(user.preferred_ethnicity, {
                lookupTables: ['user_ethnicities', 'ethnicities']
            }),
            formatPreferenceDisplay(user.preferred_occupation, {
                lookupTables: ['user_occupation_categories', 'occupation_categories', 'occupations']
            }),
            formatPreferenceDisplay(user.preferred_income, {
                lookupTables: ['user_income_ranges', 'income_ranges', 'income']
            }),
            formatPreferenceDisplay(user.preferred_marital_status, {
                lookupTables: ['user_marital_statuses', 'marital_statuses']
            }),
            formatPreferenceDisplay(user.preferred_lifestyle, {
        ...
        const userData = {
            ...
            income: user.income_name || 'Not specified',
            incomeName: user.income_name || 'Not specified',
            ...
            preferredCountries: Buffer.from(JSON.stringify(preferredCountries)).toString('base64'),
                        ...
                        userAvatar: ((user.gender || '').toLowerCase().trim() === 'f' || (user.gender || '').toLowerCase().trim() === 'female')
      ...
      height: heightDisplay,
            ...
            height_cm: isHeightOther ? '0' : (user.height_cm || ''),
            height_m: heightMeters,
            height_ft: heightFeet,
            height_in: heightInches,
            height_reference_id: isHeightOther ? '0' : (user.height_reference_id || ''),
            current_height_reference_id: isHeightOther ? '0' : (user.height_reference_id || ''),
            current_height_cm: isHeightOther ? '0' : (user.height_cm || ''),
            weight_kg: (user.weight_kg === 0 || user.weight_kg === '0') ? '0' : (user.weight_kg || ''),
            weight_reference_id: (user.weight_kg === 0 || user.weight_kg === '0') ? '0' : (user.weight_reference_id || ''),
            current_weight_reference_id: (user.weight_kg === 0 || user.weight_kg === '0') ? '0' : (user.weight_reference_id || ''),
            current_weight_kg: (user.weight_kg === 0 || user.weight_kg === '0') ? '0' : (user.weight_kg || ''),
            weightDisplay: (user.weight_kg === 0 || user.weight_kg === '0')
                ? 'Other'
                : ((user.weight_kg || user.weight_reference_id)
      ...
      company: user.company || 'Not specified',
             smokingId: user.smoking_id != null ? String(user.smoking_id) : '',
             smoking: pickDisplay(user.smoking_preference_name, user.smoking_id),
             smokingName: pickDisplay(user.smoking_preference_name, user.smoking_id),
            drinkingId: user.drinking_id != null ? String(user.drinking_id) : '',
            drinking: pickDisplay(user.drinking_preference_name, user.drinking_id),
            drinkingName: pickDisplay(user.drinking_preference_name, user.drinking_id),
            ...
            preferredWeightDisplay: (user.preferred_weight === '0' || user.preferred_weight === 0)
                ...
                : (user.preferred_weight && user.preferred_weight.trim() !== '')
                    ...
                    : 'Not specified',
            preferredExercise: preferredDisplayValues.preferredExerciseDisplay,
            preferredBodyTypeId: user.preferred_body_type != null ? String(user.preferred_body_type) : '',
            preferredBodyType: preferredDisplayValues.preferredBodyTypeDisplay,
            preferredEducation: preferredDisplayValues.preferredEducationDisplay,
            preferredEducationId: user.preferred_education != null ? String(user.preferred_education) : '',
            preferredReligion: preferredDisplayValues.preferredReligionDisplay,
            preferredReligionId: user.preferred_religion != null ? String(user.preferred_religion) : '',
            preferredSmoking: preferredDisplayValues.preferredSmokingDisplay,
            preferredSmokingId: user.preferred_smoking != null ? String(user.preferred_smoking) : '',
            preferredDrinking: preferredDisplayValues.preferredDrinkingDisplay,
            preferredDrinkingId: user.preferred_drinking != null ? String(user.preferred_drinking) : '',
            preferredChildren: preferredDisplayValues.preferredChildrenDisplay,
            preferredChildrenId: user.preferred_children != null ? String(user.preferred_children) : '',
            preferredNumberOfChildren: preferredDisplayValues.preferredNumberOfChildrenDisplay === 'Not specified'
                ? ''
                : preferredDisplayValues.preferredNumberOfChildrenDisplay,
            preferredNumberOfChildrenId: user.preferred_number_of_children != null ? String(user.preferred_number_of_children) : '',
            preferredEyeColor: preferredDisplayValues.preferredEyeColorDisplay,
            preferredHairColor: preferredDisplayValues.preferredHairColorDisplay,
            preferredEthnicity: preferredDisplayValues.preferredEthnicityDisplay,
            preferredOccupation: preferredDisplayValues.preferredOccupationDisplay,
            preferredIncome: preferredDisplayValues.preferredIncomeDisplay,
            preferredMaritalStatus: preferredDisplayValues.preferredMaritalStatusDisplay,
            relationshipType: relationshipTypeDisplay,
            preferredLifestyle: preferredDisplayValues.preferredLifestyleDisplay,
            preferredBodyArt: preferredDisplayValues.preferredBodyArtDisplay,
            preferredEnglishAbility: preferredDisplayValues.preferredEnglishAbilityDisplay,
            partnerPreferences: user.partner_preferences || '',
        ...
        const imagesResult = await this.db.query(
            `
                        SELECT id, file_name, is_profile, uploaded_at, thumbnail_small_path, thumbnail_medium_path
                        FROM user_images 
                        WHERE user_id = $1 
            ...
            [userId]
                    ...
                    const isProfileFlag = (value) => value === true || value === 1 || value === '1' || value === 'true';
                    const profilePhoto = imagesResult.rows.find(photo => isProfileFlag(photo.is_profile)) || imagesResult.rows[0];
                    if (profilePhoto && profilePhoto.file_name) {
                        const normalizeWebPath = (rawPath) => {
                        ...
                        const avatarSmall = normalizeWebPath(profilePhoto.thumbnail_small_path);
                        const avatarMedium = normalizeWebPath(profilePhoto.thumbnail_medium_path);
                        const avatarOriginal = `/uploads/profile_images/${profilePhoto.file_name}`;
                        userData.userHasProfileImage = true;
                        userData.userAvatarSmall = avatarSmall;
                        userData.userAvatarMedium = avatarMedium;
                        userData.userAvatarOriginal = avatarOriginal;
                        // Default img src: smallest suitable file (header / talk LCP); fall back chain uses data-* attrs.
                        userData.userAvatar = avatarSmall || avatarMedium || avatarOriginal;
                    }
        ...
        // Visible incoming likes only (same rules as Activity → Who liked me)
        try {
            const likeCountResult = await this.db.query(`
                SELECT COUNT(DISTINCT l.liked_by) AS like_count
                FROM users_likes l
        ...
        // Distinct viewers excluding hidden (same rules as Activity → Who viewed me total)
        try {
            const viewCountResult = await this.db.query(`
                SELECT COUNT(DISTINCT pv.viewer_id) AS view_count
            ...
            // Fallback to legacy user_activity table if new table is unavailable
            try {
                const legacyViewCountResult = await this.db.query(`
                    SELECT COUNT(DISTINCT user_id) as view_count
                    FROM user_activity
                    WHERE target_user_id = $1
                    AND activity_type = 'profile_view'
                `, [userId]);
                userData.viewCount = parseInt(legacyViewCountResult.rows[0]?.view_count || 0);
            } catch (_legacyError) {
                userData.viewCount = 0;
            }
        }

        if (templateName === 'profile-stats') {
            const profileStatsPayload = await this.buildProfileStatsPayload(userId, userData);
            userData.profileStatsJson = JSON.stringify(profileStatsPayload.data);
            userData.statsLastUpdated = profileStatsPayload.lastUpdated;
        }
    ...
    async calculateProfileCompletion(userId) {
        try {
            ...
            const uaColumns = [
                'about_me', 'height_cm', 'weight_kg', 'smoking_id', 
                'drinking_id', 'exercise_habits_id', 'ethnicity_id', 
                'income_id', 'marital_status_id', 'lifestyle_id', 'living_situation_id', 
                'have_children', 'about_partner', 'body_art_id', 
                'english_ability_id', 'relocation_id', 'occupation_category_id', 
                'body_type_id', 'eye_color_id', 
            ...
            const upColumns = [
                ...
                'preferred_english_ability', 'relationship_type_id'
            ];
            ...
            const upNotNullCount = upColumns.map(col => {
                ...
                if (['preferred_weight', 'preferred_exercise', 'preferred_children'].includes(col)) {
                    return `CASE WHEN up.${col} IS NOT NULL AND up.${col} != '' AND TRIM(up.${col}) != '' THEN 1 ELSE 0 END`;
                }
                if (col === 'relationship_type_id') {
                    return `CASE WHEN up.relationship_type_id IS NOT NULL THEN 1 ELSE 0 END`;
                }
    ...
    async buildProfileStatsPayload(userId, userData) {
        const baseStats = {
            profileViews: Number(userData.viewCount) || 0,
            likesReceived: Number(userData.likeCount) || 0,
            messagesSent: 0,
            messagesReceived: 0,
            matchesTotal: 0,
            favoritesCount: 0
        ...
        try {
            const activityStats = await this.getProfileActivityStats(userId);
            const statsData = {
                profileViews: baseStats.profileViews,
                likesReceived: baseStats.likesReceived,
                messagesSent: activityStats.messagesSent,
                messagesReceived: activityStats.messagesReceived,
                matchesTotal: activityStats.matchesTotal,
                favoritesCount: activityStats.favoritesCount
            };
    ...
    async getProfileActivityStats(userId) {
        try {
            const [
                messagesSentResult,
                ...
                this.db.query(`
                    ...
                    FROM users_likes l1
                    INNER JOIN users_likes l2 ON l1.liked_by = l2.liked_user_id 
            ...
            return {
                messagesSent: parseInt(messagesSentResult.rows[0]?.count || 0, 10),
                messagesReceived: parseInt(messagesReceivedResult.rows[0]?.count || 0, 10),
                matchesTotal: parseInt(matchesResult.rows[0]?.count || 0, 10),
                favoritesCount: parseInt(favoritesCountResult.rows[0]?.count || 0, 10)
            };
        } catch (error) {
            return {
                messagesSent: 0,
