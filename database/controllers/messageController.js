...
const { ensureAccountStatusSchema, getUserAccountStatus } = require('../../utils/accountStatus');
...
class MessageController {
    ...
    resolveConversationAvatarPath(profileImage) {
        ...
        if (/^https?:\/\//i.test(raw)) {
            try {
                ...
                if (p.startsWith('/uploads/') || p.startsWith('/assets/')) {
                    return this.resolveConversationAvatarPath(p);
                }
            } catch (_e) {
                /* ignore */
            }
        ...
        const normalized = raw.startsWith('/') ? raw : (raw.startsWith('uploads/') ? `/${raw}` : raw);

        if (normalized.startsWith('/uploads/')) {
            if (!normalized.startsWith('/uploads/profile_images/')) {
                return normalized;
            }
            if (normalized.includes('_thumb_small.')) {
                return normalized;
            }
            const swapped = normalized.replace(/_thumb_(medium|normal)(\.\w+)$/i, '_thumb_small$1');
            if (swapped !== normalized) {
                return swapped;
            }
            return normalized.replace(/(\.[a-zA-Z0-9]+)$/, '_thumb_small$1');
        }

        // Bare filename from DB: always build small thumbnail path for talk avatars.
        const basePath = `/uploads/profile_images/${raw}`;
        if (basePath.includes('_thumb_small.')) {
            return basePath;
        }
        return basePath.replace(/(\.[a-zA-Z0-9]+)$/, '_thumb_small$1');
    }
    ...
    async sendMessage(req, res) {
        try {
            ...
            const senderAccountStatus = await getUserAccountStatus(this.db, senderId);
            if (!senderAccountStatus) {
            ...
            if (senderAccountStatus.isSuspended) {
            ...
            if (senderAccountStatus.isPaused) {
                return res.status(423).json({
                    success: false,
                    error: 'Your account is paused. Resume it to send messages.',
                    code: 'ACCOUNT_PAUSED'
                });
            }
            ...
            const receiverAccountStatus = await getUserAccountStatus(this.db, finalReceiverId);
            if (!receiverAccountStatus) {
                return res.status(404).json({
                    success: false,
                    error: 'User not found',
            ...
            if (receiverAccountStatus.isSuspended) {
                return res.status(403).json({
                    success: false,
                    error: 'This user is not available for new messages right now.',
                    code: 'RECEIVER_SUSPENDED'
                });
            }
            if (receiverAccountStatus.isPaused) {
                return res.status(403).json({
                    success: false,
                    error: 'This user is not available for new messages right now.',
    ...
    async setMessageReaction(req, res) {
        try {
            const { messageId } = req.params;
            const userId = req.headers['x-user-id'] || req.body.userId;
            const reactionEmoji = (req.body.reactionEmoji || '').trim();
            ...
            if (!reactionEmoji) {
                return res.status(400).json({ success: false, error: 'reactionEmoji is required' });
            }

            if (reactionEmoji.length > 16) {
                return res.status(400).json({ success: false, error: 'reactionEmoji too long' });
            }

            const messageCheck = await this.db.query(
                `SELECT id, sender_id, receiver_id, recall_type,
            ...
            const isParticipant = parseInt(messageRow.sender_id) === numericUserId || parseInt(messageRow.receiver_id) === numericUserId;
            if (!isParticipant) {
                return res.status(403).json({ success: false, error: 'Access denied' });
            }

            const senderUserId = parseInt(messageRow.sender_id, 10);
            const receiverUserId = parseInt(messageRow.receiver_id, 10);
            const reactedByUserId = numericUserId;
            const reactedToUserId = reactedByUserId === senderUserId ? receiverUserId : senderUserId;

            if (messageRow.recall_type === 'hard') {
                return res.status(400).json({ success: false, error: 'Cannot react to recalled message' });
            }

            if ((parseInt(messageRow.sender_id) === numericUserId && messageRow.deleted_by_sender) ||
                (parseInt(messageRow.receiver_id) === numericUserId && messageRow.deleted_by_receiver)) {
                return res.status(400).json({ success: false, error: 'Cannot react to deleted message' });
            }

            await this.db.query(
                `INSERT INTO user_icon_reaction (message_id, user_id, reaction_emoji)
                 VALUES ($1, $2, $3)
                 ON CONFLICT (message_id, user_id, reaction_emoji)
                 DO NOTHING`,
                [numericMessageId, numericUserId, reactionEmoji]
            );

            const reactionSummary = await this.db.query(
                `SELECT reaction_emoji as emoji, COUNT(*)::int as count
                 FROM user_icon_reaction
                 WHERE message_id = $1
            ...
            const buildPayloadForUser = async (targetUserId) => {
                const targetReactions = await this.db.query(
                    `SELECT reaction_emoji
                     FROM user_icon_reaction
                     WHERE message_id = $1 AND user_id = $2
                     ORDER BY reaction_emoji`,
                    [numericMessageId, targetUserId]
                );

                const myReactions = targetReactions.rows
                    .map(row => row.reaction_emoji)
                    .filter(value => typeof value === 'string' && value.trim())
                    .map(value => value.trim());

                return {
                    messageId: numericMessageId,
                    reacted_by_user_id: reactedByUserId,
                    reacted_to_user_id: reactedToUserId,
                    message_sender_id: senderUserId,
                    message_receiver_id: receiverUserId,
                    my_reaction: myReactions[0] || null,
                    my_reactions: myReactions,
                    reactions: reactionSummary.rows
                };
            };

            const senderPayload = await buildPayloadForUser(senderUserId);
            const receiverPayload = await buildPayloadForUser(receiverUserId);
            const actorPayload = senderUserId === numericUserId ? senderPayload : receiverPayload;

            if (this.io) {
                this.io.to(`user_${messageRow.sender_id}`).emit('message_reaction_updated', senderPayload);
                this.io.to(`user_${messageRow.receiver_id}`).emit('message_reaction_updated', receiverPayload);
            }

            return res.json({ success: true, data: actorPayload });
        } catch (error) {
    ...
    async removeMessageReaction(req, res) {
        try {
            ...
            const reactionEmoji = (req.body.reactionEmoji || req.query.reactionEmoji || '').trim();
            ...
            if (reactionEmoji) {
                await this.db.query(
                    `DELETE FROM user_icon_reaction
                     WHERE message_id = $1 AND user_id = $2 AND reaction_emoji = $3`,
                    [numericMessageId, numericUserId, reactionEmoji]
                );
            } else {
                await this.db.query(
                ...
                result = await this.db.query(`
                    WITH active_conversations AS (
                        SELECT DISTINCT 
                            ...
                            COALESCE(u.is_suspended, false) as other_is_suspended,
                            COALESCE(u.account_status, 'active') as other_account_status,
                            COALESCE(ui.thumbnail_small_path, ui.file_name, NULL) as profile_image,
...
${otherLastSeenSelect}                            COUNT(CASE 
                                ...
                                THEN 1 
                            END) as unread_count,
                            (CASE WHEN MAX(u.birthdate) IS NOT NULL THEN EXTRACT(YEAR FROM AGE(MAX(u.birthdate)))::INTEGER ELSE NULL END) as other_age,
                            CASE 
                        ...
                        LEFT JOIN LATERAL (
                            SELECT ui_choice.thumbnail_small_path, ui_choice.file_name
                    ...
                    deleted_users_cte AS (
                        SELECT DISTINCT
                            ...
                            false as other_is_suspended,
                            'active' as other_account_status,
                            ...
                            NULL::timestamptz as other_last_seen_at,
                            NULL::integer as other_age,
                            NULL::text as other_gender
                    ...
                    SELECT 
                        ...
                        final.other_email,
                        final.other_is_suspended,
                        final.other_account_status,
                        final.profile_image,
                    ...
                    FROM (
                        SELECT 
                            ...
                            ac.other_real_name,
                            ac.other_email,
                            ac.other_is_suspended,
                            ac.other_account_status,
                            CASE 
                        ...
                        SELECT 
                            ...
                            duc.other_real_name,
                            duc.other_email,
                            duc.other_is_suspended,
                            duc.other_account_status,
                            duc.profile_image,
                ...
                if (e.code === '42P01' || (e.message && e.message.includes('does not exist'))) {
                    result = await this.db.query(`
                        SELECT 
                            ...
                            conv.other_email,
                            conv.other_is_suspended,
                            conv.other_account_status,
                            conv.profile_image,
...
${otherLastSeenSelect}                                COUNT(CASE 
            ...
            for (const row of result.rows) {
                ...
                if (isDeletedUser) {
                    ...
                    avatar = this.resolveConversationAvatarPath(row.profile_image);
                } else {
                ...
                let partnerAge = null;
                if (row.other_age != null && row.other_age !== '') {
                    const parsedAge = parseInt(row.other_age, 10);
                    if (!Number.isNaN(parsedAge) && parsedAge > 0 && parsedAge < 130) {
                        partnerAge = parsedAge;
                    }
                }
                ...
                const conversation = {
                    ...
                    gender: row.other_gender != null && row.other_gender !== '' ? String(row.other_gender) : null,
                    age: partnerAge,
                    avatar: avatar,
                    ...
                    is_suspended: row.other_is_suspended === true || row.other_is_suspended === 'true' || row.other_is_suspended === 1,
                    account_status: (row.other_account_status || 'active'),
    ...
    async uploadChatImage(req, res) {
        try {
            ...
            upload.single('image')(req, res, async (err) => {
                if (err) {
                    const maxSizeLabel = typeof imageHandler.formatBytes === 'function'
                        ? imageHandler.formatBytes(imageHandler.maxFileSize)
                        : `${Math.round((imageHandler.maxFileSize || 0) / (1024 * 1024))} MB`;

                    const normalizedMessage = err.code === 'LIMIT_FILE_SIZE'
                        ? `Image too large. Maximum size: ${maxSizeLabel}`
                        : err.message;

                    return res.status(400).json({
                        success: false,
                        error: normalizedMessage || 'Upload failed'
                    });
                }
                ...
                try {
                    // Virus scan (if available) before processing
                    try {
                        ...
                        if (!scanResult.clean) {
                            ...
                            return res.status(400).json({
                                success: false,
                                error: 'The uploaded file appears to be unsafe and has been rejected'
                            });
                        }
                    } catch (scanError) {
                        await imageHandler.deleteTempFile(req.file.path);
                    ...
                    res.status(400).json({
                        success: false,
                        error: processError.message || 'Failed to process image'
                    });
                }
            });
            ...
            upload.array('images', 5)(req, res, async (err) => {
                if (err) {
                    ...
                    const normalizedMessage = err.code === 'LIMIT_FILE_SIZE'
                        ...
                        : (err.code === 'LIMIT_UNEXPECTED_FILE'
                            ? 'Too many images. Maximum 5 images per message.'
                            : err.message);
                    return res.status(400).json({
                        success: false,
                        error: normalizedMessage || 'File upload error',
                        details: {
                            message: normalizedMessage || err.message,
                            code: err.code,
                            field: err.field
                        }
                    });
                ...
                for (const file of req.files) {
                    try {
                        ...
                        // Virus scan each file (if available) before processing
                        try {
                ...
                if (results.length === 0) {
                    const reasonSummary = Array.isArray(errors) && errors.length > 0
                        ? errors
                            .map((e) => {
                                const filename = e && e.filename ? String(e.filename) : 'file';
                                const message = e && e.error ? String(e.error) : 'Rejected';
                    ...
                    return res.status(400).json({
                        success: false,
                        error: reasonSummary
                            ? `No images were accepted. ${reasonSummary}`
                            : 'No images were accepted',
                        data: {
                            processed: [],
                            errors,
                            totalProcessed: 0,
        ...
        try {
            ...
            try {
                if (this.messageService && this.messageService.redis) {
                    // Clear the exact Redis key shape used by MessageService.getConversation.
                    const senderId = parseInt(message.sender_id, 10);
                    const receiverId = parseInt(message.receiver_id, 10);
                    const chatRoomId = typeof this.messageService.getChatRoomId === 'function'
                        ? this.messageService.getChatRoomId(senderId, receiverId)
                        : `chat:${Math.min(senderId, receiverId)}:${Math.max(senderId, receiverId)}`;

                    await this.messageService.redis.del(`${chatRoomId}:messages`);

                    // Also clear legacy key variants if they exist to avoid stale reads.
                    await this.messageService.redis.del(`${senderId}_${receiverId}:messages`);
                    await this.messageService.redis.del(`${receiverId}_${senderId}:messages`);

                    console.log(`🗑️ Cleared Redis cache for conversation ${chatRoomId} after recall`);
                }
            } catch (cacheError) {
