...
class TemplateController {
    ...
    resolveAvatarPath(imagePath) {
        if (!imagePath || typeof imagePath !== 'string') {
            return '/assets/images/default_profile.svg';
        }
        ...
        if (trimmed.startsWith('/uploads/') || trimmed.startsWith('/assets/')) {
            if (trimmed.startsWith('/uploads/profile_images/') && !trimmed.includes('_thumb_small.')) {
                return trimmed.replace(/\.[^./]+$/, '') + '_thumb_small.jpg';
            }
            return trimmed;
        }
        ...
        if (trimmed.startsWith('uploads/')) {
            ...
            if (normalized.startsWith('/uploads/profile_images/') && !normalized.includes('_thumb_small.')) {
                return normalized.replace(/\.[^./]+$/, '') + '_thumb_small.jpg';
            }
            return normalized;
        }

        const basePath = `/uploads/profile_images/${trimmed}`;
        if (!basePath.includes('_thumb_small.')) {
            return basePath.replace(/\.[^./]+$/, '') + '_thumb_small.jpg';
        }
        return basePath;
    }
            ...
            if (userId) {
                const userResult = await this.db.query(`
                          SELECT u.*, ua.*, up.*, c.name as city_name, s.name as state_name, co.name as country_name,
                              COALESCE(ui.thumbnail_small_path, ui.file_name) as profile_image
                    FROM users u
                ...
                if (userResult.rows.length > 0) {
                    ...
                    userData = {
                        ...
                        userAvatar: this.resolveAvatarPath(user.profile_image),
                ...
                if (userResult.rows.length > 0) {
                    ...
                    userData = {
                        ...
                        userLocation: dbUser.city_name || dbUser.state_name || dbUser.country_name ? 
                            ...
                            'Location not specified',
                        userAvatar: this.resolveAvatarPath(dbUser.profile_image),
