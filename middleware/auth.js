...
const { isOriginAllowed } = require('../server/config/corsOrigins');
...
class AuthMiddleware {
    ...
    // CORS middleware (legacy helper — prefer Express createPerRequestCorsMiddleware)
    cors(req, res, next) {
        ...
        res.header(
            ...
            'Origin, X-Requested-With, Content-Type, Accept, Authorization, X-Session-Token, X-CSRF-Token'
        );
    ...
    errorHandler(err, req, res, next) {
        ...
        const expose =
            !isProduction && process.env.EXPOSE_ERROR_DETAILS_IN_RESPONSE !== 'false';

        res.status(err.status || 500).json({
            success: false,
            error: expose ? err.message || 'Internal server error' : 'Internal server error',
            ...(expose && err.stack ? { stack: err.stack } : {})
        });
    }
