...
class AuthRoutes {
    ...
    setupRoutes() {
        ...
        const templateRoutes = ['billing', 'profile-full', 'profile-edit', 'profile-photos', 'search', 'results', 'activity'];
        ...
        router.get('/profile/:userId', async (req, res) => {
            ...
            if (token && this.templateController && this.authMiddleware) {
                ...
                if (session && session.expiresAt > Date.now()) {
                    session.expiresAt = Date.now() + SESSION_DURATION_MS;
                    await this.templateController.renderTemplate(req, res, 'profile-full', userId, session.user, token);
                } else {
                    res.redirect('/login?message=' + encodeURIComponent('Your session has expired. Please log in again.'));
                }
            } else {
