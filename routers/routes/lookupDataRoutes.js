        ...
        try {
            ...
            try {
                ...
                const [
                    ...
                    executeQuery("SELECT id, name FROM user_marital_statuses WHERE is_active = true ORDER BY CASE WHEN LOWER(name) = 'prefer not to say' THEN 1 ELSE 0 END ASC, display_order ASC, name ASC", 'user_marital_statuses', 'Marital statuses lookup'),
                ...
                // Load current user's preferred countries from DB (primary table + fallback column)
                ...
                if (sessionUserId) {
                    ...
                    try {
                        const preferredCountriesResult = await authController.db.query(
                            `SELECT upc.country_id AS id, c.name, c.emoji
                        ...
                        preferredCountries = preferredCountriesResult.rows;
                    } catch (preferredCountriesError) {
                        preferredCountries = [];
                    }

                    lookupData.preferred_countries = preferredCountries;
                    lookupData.preferredCountries = preferredCountries;
                }
