 ...
 * - GET /suspended - Suspended account page
...
function createPageRoutes(templateController, authMiddleware) {
    ...
    router.get('/suspended', (req, res) => {
        try {
            const suspendedPath = path.join(__dirname, '..', '..', 'app', 'pages', 'suspended.html');
            if (fs.existsSync(suspendedPath)) {
                res.sendFile(suspendedPath);
            } else {
                res.status(403).send('<h1>Account suspended</h1>');
            }
        } catch (error) {
            res.status(403).send('<h1>Account suspended</h1>');
        }
    ...
    router.get('/help', async (req, res) => {
        try {
            await renderTemplatePage(req, res, 'help');
        } catch (error) {
            console.error('[PageRoutes] Error rendering help page:', error);
            res.status(500).send('<h1>Help page not available</h1>');
        }
    });
    ...
    router.get('/account', async (req, res) => {
        try {
            const token = req.query.token || req.headers['x-session-token'] || req.cookies?.sessionToken;
            const emailVerificationState = req.query.emailVerification;
            const publicVerificationStates = new Set([
                'success',
                'already-verified',
                'invalid-or-expired',
                'missing-token',
            ...
            // Allow verified-email redirects to load account template without active session.
            if (!token && publicVerificationStates.has(emailVerificationState) && templateController) {
                await templateController.renderTemplate(req, res, 'account');
                return;
            }
