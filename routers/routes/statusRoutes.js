...
function createStatusRoutes(authController, authMiddleware = null) {
     ...
     */
    router.get('/api/user-status/:userId', statusReadLimiter, async (req, res) => {
        try {
            await authController.getUserStatus(req, res);
        } catch (error) {
            console.error('[StatusRoutes] Error in getUserStatus:', error);
            res.status(500).json({ success: false, error: 'Failed to get user status' });
        }
    });
     ...
     */
    router.get('/api/user-lastseen/:userId', statusReadLimiter, async (req, res) => {
        try {
            await authController.getUserLastSeen(req, res);
        } catch (error) {
            console.error('[StatusRoutes] Error in getUserLastSeen:', error);
            res.status(500).json({ success: false, error: 'Failed to get user last seen' });
        }
    });
     ...
     */
    router.post('/api/users-online-status', statusReadLimiter, async (req, res) => {
        try {
            await authController.getUsersOnlineStatus(req, res);
        } catch (error) {
            console.error('[StatusRoutes] Error in getUsersOnlineStatus:', error);
            res.status(500).json({ success: false, error: 'Failed to get users online status' });
        }
    });
     ...
     * Set user offline
     */
    router.post('/api/user-offline', statusWriteLimiter, async (req, res) => {
        try {
            await authController.updateUserOffline(req, res);
        } catch (error) {
            console.error('[StatusRoutes] Error in updateUserOffline:', error);
            res.status(500).json({ success: false, error: 'Failed to set user offline' });
        }
    });
     ...
     */
    router.post('/api/user-online', statusWriteLimiter, async (req, res) => {
    ...
    router.post(
        ...
        async (req, res) => {
            try {
                ...
                console.error('[StatusRoutes] Error in handlePresenceHeartbeat:', error);
                res.status(500).json({ success: false, error: 'Failed to handle heartbeat' });
            }
        }
    );
     ...
     */
    router.post('/api/user-logout', statusWriteLimiter, async (req, res) => {
        try {
            await authController.handleUserLogout(req, res);
        } catch (error) {
            console.error('[StatusRoutes] Error in handleUserLogout:', error);
            res.status(500).json({ success: false, error: 'Failed to handle logout' });
        }
    });
