...
const { ensureAccountStatusSchema } = require('../../utils/accountStatus');
...
function createUserRoutes(authController, templateController = null, authMiddleware = null) {
    ...
    router.get('/api/user/:userId', async (req, res) => {
        ...
        try {
            await ensureAccountStatusSchema(authController.db);
    ...
    router.get('/api/user/:userId/images', async (req, res) => {
        ...
        try {
            const result = await authController.db.query(`
                SELECT id, file_name, is_profile, featured, uploaded_at, thumbnail_path, thumbnail_small_path, thumbnail_medium_path
                FROM user_images 
                WHERE user_id = $1 
            ...
            const images = result.rows.map(row => ({
                ...
                thumbnail_small_path: row.thumbnail_small_path || null,
                thumbnail_medium_path: row.thumbnail_medium_path || null
            }));
