...
class MessageService {
    ...
    async getConversation(userId1, userId2, limit = 10, offset = 0) {
        ...
        const normalizeReactionList = (value) => {
        ...
        const normalizeReactionSummary = (value) => {
            return parseJsonArray(value)
                .filter(item => item && typeof item === 'object' && typeof item.emoji === 'string' && item.emoji.trim())
                .map(item => ({
        ...
        try {
            ...
            const criticalCheckQuery = `
                SELECT 
                    ...
                    (
                        SELECT COUNT(*)
                        FROM user_icon_reaction r
                        JOIN user_messages rm ON rm.id = r.message_id
                        WHERE ((rm.sender_id = $1 AND rm.receiver_id = $2) OR (rm.sender_id = $2 AND rm.receiver_id = $1))
                    ) as reaction_count
                FROM user_messages 
            ...
            const hasReactions = parseInt(criticalCheck.rows[0].reaction_count) > 0;
            ...
            if (!hasAttachments && !hasRecalledMessages && !hasReactions && this.redis && safeOffset === 0 && safeLimit <= 100) {
            ...
            const result = await this.db.query(`
                SELECT * FROM (
                    SELECT m.id, m.sender_id, m.receiver_id, m.message, m.timestamp, m.status,
                           ...
                           COALESCE((
                               SELECT json_agg(r.reaction_emoji ORDER BY r.reaction_emoji)
                               FROM user_icon_reaction r
                               WHERE r.message_id = m.id AND r.user_id = $1
                           ), '[]'::json) as my_reactions,
                           (
                               SELECT r.reaction_emoji
                               FROM user_icon_reaction r
                               WHERE r.message_id = m.id AND r.user_id = $1
                               ORDER BY r.reaction_emoji
                               LIMIT 1
                           ) as my_reaction,
                           COALESCE((
                               SELECT json_agg(
                                   json_build_object('emoji', grouped.reaction_emoji, 'count', grouped.reaction_count)
                                   ORDER BY grouped.reaction_emoji
                               )
                               FROM (
                                   SELECT r2.reaction_emoji, COUNT(*)::int as reaction_count
                                   FROM user_icon_reaction r2
                                   WHERE r2.message_id = m.id
                                   GROUP BY r2.reaction_emoji
                               ) grouped
                           ), '[]'::json) as reactions_summary,
            ...
            for (const row of result.rows) {
                const myReactions = normalizeReactionList(row.my_reactions);
                const reactionSummary = normalizeReactionSummary(row.reactions_summary);
                ...
                const message = {
                    ...
                    my_reaction: myReactions[0] || row.my_reaction || null,
            ...
            if (!hasAttachments && !hasReactions && this.redis && offset === 0 && messages.length > 0) {
                const pipeline = this.redis.multi();
