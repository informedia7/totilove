        ...
        try {
            ...
            for (const image of userImagesResult.rows) {
                if (image.file_name) {
                    ...
                    const normalizedOriginal = normalizeProfileImagePath(image.file_name);
                    if (normalizedOriginal) {
                        originalCandidates.add(normalizedOriginal);
                    }

                    const originalBaseName = path.basename(String(image.file_name || ''));
                    if (originalBaseName) {
                        originalCandidates.add(path.join(profileImagesDir, originalBaseName));
                    }

                    for (const originalPath of originalCandidates) {
                        await recordDelete(originalPath);
                    }
                    ...
                    [
                        ...
                        image.thumbnail_small_path,
                        image.thumbnail_medium_path
                    ].forEach((thumbValue) => {
                        if (!thumbValue) return;
                        addThumbCandidate(thumbValue);
                        addThumbCandidate(path.basename(thumbValue));
                    });
