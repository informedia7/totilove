...
const { ensureAccountStatusSchema, getUserAccountStatus } = require('./accountStatus');
            ...
            socket.on('send_message', async (data) => {
                ...
                try {
                    ...
                    if (senderAccountStatus.isPaused) {
                        socket.emit('error', {
                            message: 'Your account is paused. Resume it to send messages.',
                            code: 'ACCOUNT_PAUSED'
                        });
                        return;
                    }
                    ...
                    const receiverAccountStatus = await getUserAccountStatus(this.db, receiverId);
                    ...
                    if (receiverAccountStatus.isSuspended) {
                        socket.emit('error', {
                            message: 'This user is not available for new messages right now.',
                    ...
                    // Redis-based rate limiting (see config/rateLimits.js)
                    const userKey = `msg_rate_${senderId}`;
                    const rateLimit = this.monitoring.isHighLoad
                        ? rateLimits.WS_CHAT_MESSAGES_PER_MINUTE_HIGH_LOAD
                        : rateLimits.WS_CHAT_MESSAGES_PER_MINUTE_NORMAL;
                    let userCount = 0;
                    try {
                        userCount = await this.redis.getClient().incr(userKey);
                        if (userCount === 1) {
                            await this.redis.getClient().expire(userKey, rateLimits.WS_CHAT_RATE_WINDOW_SECONDS);
                        }
                    } catch (err) {
